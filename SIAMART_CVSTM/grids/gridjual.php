...
echo("<th rowspan=2> No. </th>");$mwd1='2 readonly';
echo("<th rowspan=2> Kode<br>[F5]</th>");$mwd2='8';
echo("<th rowspan=2> Uraian </th>");$mwd3='45 readonly';
echo("<th colspan=4> Qty jual </th>");$mwd4567='6';
echo("<th colspan=4 hidden> Extra </th>");$mwd891011='6';
echo("<th rowspan=2 width='63px'> Harga </th>");$mwd12='7 readonly';
echo("<th rowspan=2> Disc.1<br>% </th>");$mwd12a='7 readonly';
echo("<th rowspan=2> Disc.2<br>Rp.</th>");$mwd13='7';
echo("<th rowspan=2> Jumlah </th>");$mwd14='12 readonly';
echo("<th rowspan=2> isi </th>");$mwd15='5 readonly';
echo("</tr>");

echo("<tr>");

echo("<th colspan=2> Qty </th>");$mwd4='6';
$mwd5='4 readonly';
echo("<th colspan=2> Unit </th>");$mwd6='4';
$mwd7='4 readonly';

echo("<th colspan=2 hidden> Qty </th>");$mwd8='6 disabled';
$mwd9='4 readonly';
echo("<th  colspan=2 hidden> Unit </th>");$mwd10='4  disabled';
$mwd11='4 readonly';
...
<th   hidden><input size=$mwd8 type='text' readonly $msty></th>
<th   hidden><input size=$mwd9 type='text' readonly $msty></th>
<th   hidden><input size=$mwd10 type='text' readonly $msty></th>
...
FORMAT(hrgsat,0) hrgsat,FORMAT(diskonrp,0) diskonpros,FORMAT(diskonp,0) diskonrp,FORMAT(jmlhrg,0) jmlhrg
from transjual2 a 
where a.nid='$mnid' 
order by nomor";
...
while ($rows=mysql_fetch_assoc($rrw))
	{
		...
		$mtoid=$rows[stoid];
		$nng=execute("select concat(stonama,' (',isppn,')') stonama,satuan1,satuan2,isi from setstok where stoid='$mtoid'");
		...
		value='".$rows[qty]."' $nstt2 onblur='cekqty(this)'></td>
		";
		...
		value='".$rows[unit]."' $nstt2  onblur='cekqty(this)' ></td>
		";
		...
		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td hidden><input type='text' $midname size=$mwd8
		value='".$rows[extra]."' $nstt2 ></td>
		";
		
		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td hidden><input type='text' $midname size=$mwd9
		value='".$nng[satuan1]."' $nstt ></td>
		";

		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td hidden><input type='text' $midname size=$mwd10
		value='".$rows[extraunit]."' $nstt2 ></td>
		";
		
		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td hidden><input type='text' $midname size=$mwd11
		value='".$nng[satuan2]."' $nstt ></td>
		";
		...
		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td><input type='text' $midname size=$mwd12a
		value='".$rows[diskonpros]."' $nstt2 ></td>
		";

		$midname="id='diskrp".'_'."$mnom'  name='$diskrp".'_'."$mnom'";		
		echo "	
		<td><input type='text' $midname size=$mwd13
		value='".$rows[diskonrp]."' $nstt2 ></td>
		";
		...
		$mjrow=$mjrow+1;$midname="id='misi".'_'."$mnom'  name='$misi".'_'."$mnom'";		
		echo "	
		<td><input type='text' $midname size=$mwd15
		value='".$nng[isi]."' $nstt2 ></td>
		";
		echo "</tr>";
		$mnom++;
	}
	...
	<tr>
	<!--<td  bgcolor='white' colspan=2 align=right hidden>Ongkos&nbsp;&nbsp;</td>
	<td  bgcolor='white'><input size=12 type='text'  $nstt2 id='mongkos' hidden></td> -->
	
	<td  bgcolor='white' colspan=2 align=right>Potongan&nbsp;&nbsp;</td>
	<td  bgcolor='white'><input size=12 type='text'  $nstt2 id='mdiskon'></td>
	</tr>
	...
	<td   hidden bgcolor='white'><input size=$mwd8 type='text'  $msty readonly></td>
	<td   hidden bgcolor='white'><input size=$mwd9 type='text'  $msty readonly></td>
	<td   hidden bgcolor='white'><input size=$mwd10 type='text'  $msty readonly></td>
