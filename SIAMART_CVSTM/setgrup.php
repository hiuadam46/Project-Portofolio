	...
	$Pcs2(document).ready(function()
	{
	  ...
	  $Pcs2(document).keydown(function(){
			...
			$Pcs2.ajax(
			{
				...
				success: function(data)
				{
					...
					for (var mii=1;mii<30;mii++)
					{
						...
						//alert ("nilai maj.value = "+ maj.value)
					}
				}
			});
        ...
        $Pcs2("#btnsimpan").click(function() 
        {
			...
			execajaxa("insert into setgrp (grpid, grpnama, rekid, qty, diskon, manid) values('"+msu+"', '"+$Pcs2("#mgrpnama").val()+"', '"+$Pcs2("#mrekid").val()+"', '"+$Pcs2("#mqty").val()+"', '"+$Pcs2("#mdiskon").val()+"', '"+$Pcs2("#mmanid").val()+"')");
...
//    SyntaxHighlighter.all();
   ...
   var mws=screen.width
	...
	url: "item.php",
	request: "msql=select a.*,b.reknama,true as edit from setgrp a left join setrek b on a.rekid=b.rekid where (1=1) order by grpid"
    };
