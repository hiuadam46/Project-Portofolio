...
for ($abc=1;$abc<500;$abc++)

{

$result = mysql_query ($sqlstr.' limit '.$mawl.',100') or die ("Kesalahan pada perintah SQL JUAL1! ".mysql_error());
...
$maww=execute2("select sum(debet-kredit) awal,sum(qtyin-qtyout) awalq from bkbesar where tgl<str_to_date('$mtg1','%d-%c-%Y') and rekid like '103%' and bpid2 like '$mlokid%' and bpid='$row->stoid'");
$mtrr=execute2("select sum(debet) debet,sum(kredit) kredit,sum(qtyin) debetq,sum(qtyout) kreditq from bkbesar where ( tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') )and rekid like '103%' and bpid2 like '$mlokid%' and bpid='$row->stoid'");

$mrekid=$row->stoid;
$mreknama=$row->stonama;
$mawal=number_format($maww->awal,0,'.',',');
$mdebet=number_format($mtrr->debet,0,'.',',');
$mkredit=number_format($mtrr->kredit,0,'.',',');
$row->saldo=$maww->awal+$mtrr->debet-$mtrr->kredit;
$msaldo=number_format($row->saldo,0,'.',',');

$mawalq=cqty($maww->awalq,$row->isi,$row->isi2,$row->satuan1,$row->satuan2,$row->satuan3);
$mdebetq=cqty($mtrr->debetq,$row->isi,$row->isi2,$row->satuan1,$row->satuan2,$row->satuan3);
$mkreditq=cqty($mtrr->kreditq,$row->isi,$row->isi2,$row->satuan1,$row->satuan2,$row->satuan3);
$row->saldoq=$maww->awalq+$mtrr->debetq-$mtrr->kreditq;
...
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_cvstm_data' and TABLE_NAME='$mtabelnya'
";
...
{
$msql="SELECT COLUMN_NAME kolom,DATA_TYPE tipe,NUMERIC_PRECISION panjang1,CHARACTER_MAXIMUM_LENGTH panjang2,COLUMN_COMMENT koment
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_cvstm_data' and TABLE_NAME='$mtabelnya' and COLUMN_NAME in ($mkol)
";
}
...
{
	$sqlstr="
	select c.stonama, e.lgnnama,
	case
		when e.golongan='A' then 'PROVISION' 
		when e.golongan='B' then 'WHOLE SELLER'
		when e.golongan='C' then 'SUPERMARKET'
	end as channel,
	f.karnama, date_format(a.tgl, '%d-%m-%Y') tgl, a.nid, b.totqty, ((b.hrgsat/b.isi) * ((b.qty*b.isi)+b.unit)) as jumlah, ifnull(h.nid,'') as no_retur, ifnull(h.totqty,0) as totqty_retur, ifnull(h.jmlhrg,0) as jumlah_retur, ((b.hrgsat/b.isi)*(b.diskonrp/100)*b.totqty) as diskon_rp, (b.jmlhrg-ifnull(h.jmlhrg,0)) as jumlah_dpp, ((b.jmlhrg-ifnull(h.jmlhrg,0))*0.1) as jumlah_ppn
	from transjual1 a
...
<table width=100% cellspacing=0 cellpadding=0 bgcolor=white border=1 style='border-collapse:collapse;font-size:85%'> 
	...
	<tr>
		...
		<td align=right>".number_format($msx->jumlah-$msx->jumlah_dpp,0,'.',',')."</td>
		...
		$mtotaljmldisk=$mtotaljmldisk+$msx->jumlah-$msx->jumlah_dpp;
	...
	<tr valign=>
		...
		<td align=right><b>".number_format($mtotaljumlah,0,'.',',')."</td>
		<td></td>
		<td></td>
...
$sqlstr = " select a.nid,date_format(a.tgl,'%d-%m-%Y') tanggal_faktur,lgnnama as nama,npwp,alamat,b.stoid,d.stonama,
b.jmlhrg as dpp,b.jmlhrg*0.1 as ppn,(b.hrgsat/b.isi) as hrgsat ,(b.qty*b.isi)+b.unit as qty,
((b.hrgsat/b.isi)*(b.qty*b.isi)) + ((b.hrgsat/b.isi)*b.unit) as subtotal,
(((b.hrgsat/b.isi)*(b.qty*b.isi)) + ((b.hrgsat/b.isi)*b.unit))-b.jmlhrg as diskon
from transjual1 a left join 
transjual2 b on a.nid=b.nid left join 
setlgn c on a.lgnid=c.lgnid left join 
setstok d on b.stoid=d.stoid
...
";
$nn=1;$totot=0;$tottunai=0;$tothutang=0;
while ($mrr=mysql_fetch_object($maa))
{echo "
<tr>
...
{
	/* $sqlstr="
	select a.tgl, a.ppnkeluaran, a.nid, x.jumlah, a.jumlah as jumlahdaria, a.lgnid, a.supid, a.soid, a.sales, a.mobil, a.tunaikredit, a.onreport, a.ket, a.pdiskon , y.diskon, a.tunai, a.total, a.report, a.komp, a.user, a.status, a.tgljt, a.terkirim, a.ongkos,
	case when a.nomorfp = 'undefined'
		then
			''
		else
			a.nomorfp
	...
	left join
	(
		select nid, sum((hrgsat/isi)*totqty) as jumlah
		from transjual2
		group by nid
	) x on a.nid=x.nid
	left join
	(
		select nid, sum((hrgsat/isi)*(diskonrp/100)*totqty) as diskon
		from transjual2
		group by nid
	) y  on a.nid=y.nid
	...
	"; */
	$sqlstr="
	select a.tgl, a.ppnkeluaran, a.nid, x.jumlah, a.jumlah as jumlahdaria, a.lgnid, a.supid, a.soid, a.sales, a.mobil, a.tunaikredit, a.onreport, a.ket, a.pdiskon , y.diskony, a.tunai, a.total, a.report, a.komp, a.user, a.status, a.tgljt, a.terkirim, a.ongkos, a.diskon,
	case when a.nomorfp = 'undefined'
		then
			''
		else
	...
	left join
	(
		select nid, sum((hrgsat/isi)*(diskonrp/100)*totqty) as diskony
		from transjual2
		group by nid
	) y  on a.nid=y.nid
	left join setlgn b on a.lgnid=b.lgnid
...
<table width=100% cellspacing=0 cellpadding=0 bgcolor=white border=1 style='border-collapse:collapse;font-size:85%'> 
	...
	while ($mrr=mysql_fetch_object($maa))
	{
		...
		$sisa=$mrr->total-$mrr->diskon-$mrr->tunai;
	...
	<tr valign= height=25 class='thetr' onmousemove=highl(this) >
		...
		$totsisa=$totsisa+($mrr->total-$mrr->diskon-$mrr->tunai);
		
		$totretur=$totretur+$mrr->retur;
		$totlunas=$totlunas+$mrr->lunas;
		//$totpiutang=$totpiutang+($mrr->jumlah-$mrr->diskon-$mrr->tunai-$mrr->retur-$mrr->lunas);
		$totpiutang=$totpiutang+($mrr->total-$mrr->diskon-$mrr->tunai-$mrr->retur-$mrr->lunas);
...
{
	$sqlstr="
	SELECT b.lgnid, b.lgnnama, c.stoid, c.stonama, c.hrgsat, c.qty, c.diskonrp, c.jmlhrg, c.isi, c.nid, date_format(c.tgl,'%d-%m-%Y') tgl, c.satuan1, c.satuan2, c.unit, c.supid
	FROM transjual1 a
