...
if ($mLapo=='mlapabsensi')
{
	...
	while ( date( "Y-m-d", strtotime( "$mttt")) < date( "Y-m-d", strtotime( "$mtg2")) )
	{
		...
		echo ("
			<td bgcolor=#CCCCCC align=center ><b>&nbsp;&nbsp;". date( "d-m-Y",strtotime( "$mtgt")) ."&nbsp;&nbsp;<br>WH</b></td>
		");
		$mttt=$mtgt;
		$mnnn++;
	}
	...
	while ($row = mysql_fetch_assoc ($result))
	{
		...
		$query106=executerow("select time_to_sec(jam) jam,jam jam1 from absensi where tgl='$mtg1x' and karid='$row[karid]' order by jam limit 0,1");
		$mtr1=mysql_fetch_assoc ($query106);
		
		$query109=executerow("select time_to_sec(jam) jam,jam jam2 from absensi where tgl='$mtg1x' and karid='$row[karid]' order by jam limit 1,1");
		$mtr2=mysql_fetch_assoc ($query109);
		...
		echo("
			<td bgcolor=$col align=center> $mb.$md <br><font size=1> $mtr1[jam1] - $mtr2[jam2] </font></td>
		");
		...
		while ( date( "Y-m-d", strtotime( "$mttt")) < date( "Y-m-d", strtotime( "$mtg2")) )
		{
			...
			$query141=executerow("select time_to_sec(jam) jam,jam jam1 from absensi where tgl='$mtg1x' and karid='$row[karid]' order by jam limit 0,1");
			$mtr1=mysql_fetch_assoc ($query141);
			
			$query144=executerow("select time_to_sec(jam) jam,jam jam2 from absensi where tgl='$mtg1x' and karid='$row[karid]' order by jam limit 1,1");
			$mtr2=mysql_fetch_assoc ($query144);
...
if ($mLapo=='mkrtstok')
{
	...
	$sqlstr='
	select date_format(str_to_date("'.$mtg1.'", "%d-%c-%Y"), "%d-%m-%Y") tglc, "SA" nid, "Saldo Awal" ket, sum(debet-kredit) saldorp, sum(qtyin-qtyout) saldoqty
	from bkbesar
	where tgl<str_to_date("'.$mtg1.'", "%d-%c-%Y") and rekid like "103%" and bpid="'.$mstoid.'" and bpid2 like "'.$mlokid.'%"
	';
	
	$query697 =executerow($sqlstr);
	$rowawal=mysql_fetch_assoc ($query697);
	...
	$query701 =executerow($sqlstr2);
	$rowstok=mysql_fetch_assoc ($query701);
	...
	$sqlstr3='
	select date_format(tgl, "%d-%m-%Y") tglc, bpid2, nid, ket, sum(debet) debet, sum(kredit) kredit, sum(qtyin) qtyin, sum(qtyout) qtyout, 000000000 saldoqty, 000000000 saldoharga
	from bkbesar
	where tgl between str_to_date("'.$mtg1.'", "%d-%c-%Y") and str_to_date("'.$mtg2.'", "%d-%c-%Y") and rekid like "103%" and bpid="'.$mstoid.'" and bpid2 like "'.$mlokid.'%"
	group by tgl, nid, debet
	order by ts, tgl
	';
	
	$result3 = mysql_query ($sqlstr3) or die ("Kesalahan pada perintah SQL JUAL1! ".mysql_error());
	...
	<table width=100% cellspacing=0 cellpadding=0 bgcolor=Black border=1 style="border-collapse:collapse; font-size:85%">
		...
		<tr>
			...
			<td bgcolor=#FFFFFF align=left >'.$rowawal[tglc].'</b></td>
	...
	while ($row = mysql_fetch_assoc ($result3))
	{
		...
		<tr>
			<td bgcolor=#FFFFFF align=right >'.$mnom.'.</td> 
			<td bgcolor=#FFFFFF align=left > '.$row[tglc].'</td>
...
if ($mLapo=='mkrtsup')
{
	...
	$sqlstr="
	select date_format(str_to_date('$mtg1', '%d-%c-%Y'), '%d-%m-%Y') tgl, 'SA' nid, 'Saldo Awal' ket, sum(kredit-debet) saldorp
	from bkbesar
	where tgl<str_to_date('$mtg1', '%d-%c-%Y') and rekid='20010' and bpid='$msupid'
	";
	$query841 =executerow($sqlstr);
	$rowawal=mysql_fetch_assoc ($query841);
	
	$sqlstr="select * from setsup where supid='$msupid' ";
	$query845 =executerow($sqlstr);
	$rowstok=mysql_fetch_assoc ($query845);
	...
	from bkbesar
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and rekid='20010' and bpid='$msupid'
	order by ts, tgl
	";
...
if ($mLapo=='mkrtlgn')
{
	...
	$sqlstr="
	select date_format(str_to_date('$mtg1', '%d-%c-%Y'), '%d-%m-%Y') tgl, 'SA' nid, 'Saldo Awal' ket, sum(debet-kredit) saldorp
	from bkbesar
	where tgl<str_to_date('$mtg1','%d-%c-%Y') and rekid='10210' and bpid='$mlgnid'
	";
	$query939 =executerow($sqlstr);
	$rowawal=mysql_fetch_assoc ($query939);
	
	$sqlstr="select * from setlgn where lgnid='$mlgnid' ";
	$query943 =executerow($sqlstr);
	$rowstok=mysql_fetch_assoc ($query943);
	...
	select date_format(tgl, '%d-%m-%Y') tgl, nid, ket, debet, kredit
	from bkbesar
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and rekid='10210' and bpid='$mlgnid'
	order by ts, tgl
	";
	$result2 = mysql_query ($sqlstr2) or die ("Kesalahan pada perintah SQL JUAL1! ".mysql_error());
...
if ($mLapo=='mkrtrek')
{
	...
	select date_format(str_to_date('$mtg1', '%d-%c-%Y'), '%d-%m-%Y') tgl, 'SA' nid, 'Saldo Awal' ket, sum(debet-kredit) saldorp
	from bkbesar
	where tgl<str_to_date('$mtg1', '%d-%c-%Y') and rekid='$mrekid'
	";
	$query1036 =executerow($sqlstr);
	$rowawal=mysql_fetch_assoc ($query1036);
	...
	$query1041 =executerow($sqlstr);
	$rowstok=mysql_fetch_assoc ($query1041);
	...
	select date_format(tgl, '%d-%m-%Y') tgl, nid, ket, debet, kredit
	from bkbesar
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and rekid='$mrekid'
	order by ts, tgl
	";
	$result2 = mysql_query ($sqlstr2) or die ("Kesalahan pada perintah SQL JUAL1! ".mysql_error());
...
if ($mLapo=='mSetup')
{
	...
	FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_2jaya_data' and TABLE_NAME='$mtabelnya'
	";
	...
	$query1146=executerow($msql);
	$rwwc=mysql_fetch_assoc ($query1146);
	...
	{
		$msql="SELECT COLUMN_NAME kolom,DATA_TYPE tipe,NUMERIC_PRECISION panjang1,CHARACTER_MAXIMUM_LENGTH panjang2,COLUMN_COMMENT koment
		FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_2jaya_data' and TABLE_NAME='$mtabelnya' and COLUMN_NAME in ($mkol)
		";
	}
	...
	echo "
	<br>
	<table border=1 cellspacing=1 cellpadding=0 bgcolor='grey' style='border-collapse:collapse; font-size:85%'> 
		<tr>
...
if ($mLapo=='mnrcc')
{
	...
	<table cellspacing=2 border=1 width=80%>
		<tr>
			<th width=50% colspan=2>Aset</th>
			<th width=50% colspan=2>Kewajiban dan Kekayaan</th>
		</tr>
		<tr>
	...
	while ($mrow=mysql_fetch_assoc($ttr))
	{
		echo "
					<tr>
						<td colspan=2><b>$mrow[clnama]</b></td>
					</tr>
		";
		...
		{
			$query1283=executerow("select sum(debet-kredit) jumlah from bkbesar a left join setrek b on a.rekid=b.rekid where b.nrcid='$mrow2[nrcid]' and (a.tgl<='$mtgt' or trans='SA')");
			$mjumm=mysql_fetch_assoc ($query1283);
			
			echo "
					<tr>
						<td>&nbsp;-$mrow2[nrcnama]</td>
						<td align=right> ".number_format($mjumm[jumlah],0,'.',',')." &nbsp;</td>
					</tr>
			";
		...
		echo "
					<tr>
						<td><b><hr>Total $mrow[clnama]</b></td>
						<td align=right><b><hr width=80%>".number_format($mtot,0,'.',',')." &nbsp;</b></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					</tr>
	...
	while ($mrow=mysql_fetch_assoc($ttr))
	{
		...
		while ($mrow2=mysql_fetch_assoc($ttr2))
		{
			$query1328=executerow("select sum(kredit-debet) jumlah from bkbesar a left join setrek b on a.rekid=b.rekid where b.nrcid='$mrow2[nrcid]' and (a.tgl<='$mtgt' or trans='SA')");
			$mjumm=mysql_fetch_assoc ($query1328);
			
			$mrek=$mjumm[jumlah];

			if ($mrow2[nrcid]=='213')
			{
				$query1335=executerow("select sum(kredit-debet) jumlah from bkbesar a left join setrek b on a.rekid=b.rekid left join setneraca c on b.nrcid=c.nrcid left join setklas d on c.clid=d.clid where d.posisi in ('D','B') and ( a.tgl between '$mtg1' and '$mtgt') ");
				$mjumm1=mysql_fetch_assoc ($query1335);
			...
			if ($mrow2[nrcid]=='212')
			{
				$query1343=executerow("select sum(kredit-debet) jumlah from bkbesar a left join setrek b on a.rekid=b.rekid left join setneraca c on b.nrcid=c.nrcid left join setklas d on c.clid=d.clid where d.posisi in ('D','B') and a.tgl between '$mtga' and '$mtg1s' ");
				$mjumm1=mysql_fetch_assoc ($query1343);
			...
			if ($mrow2[nrcid]=='211')
			{
				$query1351=executerow("select sum(kredit-debet) jumlah from bkbesar a left join setrek b on a.rekid=b.rekid left join setneraca c on b.nrcid=c.nrcid left join setklas d on c.clid=d.clid where d.posisi in ('D','B') and a.tgl<'$mtga' ");
				$mjumm1=mysql_fetch_assoc ($query1351);
			...
			echo "
					<tr>
						<td>&nbsp;-$mrow2[nrcnama]</td>
						<td align=right> ".number_format($mrek,0,'.',',')." &nbsp;</td>
					</tr>
			";
	...
	echo("
		...
		<tr>
			<td width=25%><b>Total Aset</td>
			<td width=25% align=right><b>".number_format($mtot2,0,'.',',')." &nbsp;</td>
			<td width=25%><b> Total Kewajiban dan Kekayaan</td>
			<td width=25% align=right><b>".number_format($mtot3,0,'.',',')." &nbsp;</td>
		</tr>
	</table>
	");
}
...
if ($mLapo=='mlapjualkasir')
{
	...
	$sqlstr="
	select a.bpid stoid, sum(a.qtyout-a.qtyin) qty, sum(a.kredit-a.debet) hpp, b.stonama, b.isi, b.satuan1, b.satuan2, sum(d.jmlhrg) jmlhrg
	from bkbesar a
	left join setstok b on a.bpid=b.stoid
	left join transkasir1 c on a.nid=c.nid
	left join transkasir2 d on a.nid=d.nid and a.bpid=d.stoid
	where a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') and b.grpid like '%$mgrpid%' and c.kasir like '!!$mkasir!!' and c.lgnid like '!!$mlgnid!!' and a.trans='TRANSKASIR' and a.rekid like '103%' and $ttk and b.supid like '$msupid%'
	group by a.bpid, if(a.qtyout>0,'A','B')
	order by $urut
	";
	...
	from (
		...
		left join transkasir1 c on a.nid=c.nid
		left join (
			select a.nid, bpid, sum(kredit-debet) hpp
			from bkbesar a
			left join transkasir1 b on a.nid=b.nid
			where b.kasir like '%$mkasir%' and rekid='10310' and kredit>0 and trans='TRANSKASIR' and a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y')
			group by bpid) d on a.stoid=d.bpid
			where a.qty>0 and a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') and b.grpid like '%$mgrpid%' and c.kasir like '!!$mkasir!!' and c.lgnid like '!!$mlgnid!!' and $ttk and b.supid like '$msupid%'
			group by a.stoid) a
	union
	select *
	from (
		...
		left join (
			...
			from bkbesar a
			left join transkasir1 b on a.nid=b.nid
			where b.kasir like '%$mkasir%' and rekid='10310' and debet>0 and trans='TRANSKASIR' and a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y')
			group by bpid) d on a.stoid=d.bpid
		where a.qty<0 and a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') and b.grpid like '%$mgrpid%' and c.kasir like '!!$mkasir!!' and c.lgnid like '!!$mlgnid!!' and $ttk and b.supid like '$msupid%'
		group by a.stoid) b
	order by $urut, qty desc
	";
	
	$query2181=executerow("select lgnnama from setlgn where lgnid='$mlgnid' ");
	$mlgg=mysql_fetch_assoc ($query2181);
	...
	$query2254=executerow("
	select sum(jumlah) jumlah, sum(diskon) diskon, sum(if (bayar<jumlah and bayar<>0,bayar,0)) um
	from transkasir1
	where tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') and kasir like '!!$mkasir!!' and lgnid like '!!$mlgnid!!' and $ttk
	");
	$wrr2=mysql_fetch_assoc ($query2254);
...
if ($mLapo=='mprintgrp')
{
	...
	$query='
	select a.stoid, FORMAT(a.qty,0) qty, FORMAT(a.hrgsat,0) hrgsat, cdiskon diskon, FORMAT(a.jmlhrg,0) jmlhrg, a.satuan, a.isi, b.stonama
	from transkasir2 a
	left join setstok b on a.stoid=b.stoid
	where a.nid="'.$mnid.'" and b.grpid like "%'.$mgrpid.'%"
	order by num
	';
	
	$query2314=executerow('
	select a.*, date_format(a.tgl, "%d-%m-%Y") tglc, b.lgnnama
	from transkasir1 a
	left join setlgn b on a.lgnid=b.lgnid
	where a.nid="'.$mnid.'"
	');
	$mlgg=mysql_fetch_assoc ($query2314);
	...
	<div style="width:51%">
		<table style="width: 100%">
			...
			<tr>
				...
				<td colspan="3">'.$mlgg[lgnnama].'</td>
			</tr>
			<tr>
				<td colspan="5">&nbsp;</td>
			</tr>
			<tr>
				<td style="width:33px">Grup</td>
				<td style="text-align:center">:</td>
				<td colspan="3">'.$_GET["mgrpnama"].'</td>
			</tr>
		</table>
	');
	...
	while ($rows=mysql_fetch_assoc($rrw))
	{
		echo '
			<tr>
				<td colspan="7">-'.$rows[stonama].'@'.$rows[isi].'</td>
			</tr>
			<tr>
				<td>&nbsp;</td>
				<td style="font-weight:bold;text-align:left">'.$rows[qty].'</td>
				<td style="font-weight:bold">'.$rows[satuan].'</td>
...
if ($mLapo=='mlapkashar')
{
	...
	$query2412=executerow("select sum(debet-kredit) saldo from bkbesar where rekid='10010' and tgl<str_to_date('$mtg1', '%d-%c-%Y') ");
	$msal=mysql_fetch_assoc ($query2412);
	
	echo "
	<table cellspacing=4>
		<tr>
			<th bgcolor=lightgrey>Uraian</th>
			<th bgcolor=lightgrey>Jumlah</th>
		</tr>
		<tr>
			<td><b>Saldo Awal</td>
			<td align=right><b>".number_format($msal[saldo],0,'.',',')."</td>
		</tr>
	";

	$mgg=executerow("
	select trans, ket, sum(debet-kredit) rp
	from bkbesar
	where rekid='10010' and tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and trans in ('TRANSKASIR', 'TRANSJUAL')
	group by trans
	union
	select trans, ket, sum(debet-kredit) rp
	from bkbesar
	where rekid='10010' and tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and trans not in ('TRANSKASIR', 'TRANSJUAL')
	group by trans, ket
	order by rp desc
	");
	echo "
		<tr>
			<td colspan=2><br>Pemasukan : </td>
		</tr>
	";
	$mtot=$msal[saldo];
	while ($roww=mysql_fetch_assoc($mgg))
	{
		...
		<tr>
			<td><hr><b>Total</td>
			<td align=right><hr><b>".number_format($mtot-$msal[saldo],0,'.',',')."</td>
		</tr>
			";
			echo "
		<tr>
			<td colspan=2><br>Pengeluaran : </td>
		</tr>
			";
			$mpem=2;
			$mtotb=0;
		...
		<tr>
			<td> - $roww[ket]</td>
			<td align=right>".number_format(abs($roww[rp]),0,'.',',')."</td>
		</tr>
		";
	...
	echo "
		<tr>
			<td><hr><b> Total </td>
			<td align=right><hr><b>".number_format($mtotb,0,'.',',')."</td>
		</tr>
	";

	echo "
		<tr>
			<td><br><b> Saldo Akhir </td>
			<td align=right><br><b>".number_format($mtot,0,'.',',')."</td>
		</tr>
	";
...
if ($mLapo=='mrrl')
{
	...
	<table cellspacing=2 border=1 width=80%>
		<tr>
			<th width=50% colspan=1> Tgl. $mtg1 s.d. $mtg2 </th>
			<th width=50% colspan=1> Total s.d. $mtg2 </th>
		</tr>
		<tr>
			<td>
	");
	...
	{
		$query2537=executerow("select sum(kredit-debet) jumlah from bkbesar where tgl between '$mtgl1' and '$mtgl2' and rekid='$rrow[rekid]'");
		$mjj=mysql_fetch_assoc ($query2537);
		
		if ($rrow[rekid]=='31010')
		{
			echo "
					<tr>
						<td><b><hr> Penjualan Bersih </td>
						<td align=right><b><hr> $mctot </td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					</tr>
		...
		if ($rrow[rekid]=='40010')
		{
			echo "
					<tr>
						<td><b><hr> Total HPP </td>
						<td align=right><b><hr> $mctot2 </td>
						<td><b>_</td>
					</tr>
					<tr>
						<td><b><hr><font color=$mcolor> $mlorr Kotor </td>
						<td align=right><b><hr><font color=$mcolor> $mctot </td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					</tr>
		...
		if ($rrow[rekid]=='40310')
		{
			echo "
					<tr>
						<td><b><hr> Total Biaya </td>
						<td align=right><b><hr> $mctot2 </td>
						<td><b>_</td>
					</tr>
					<tr color=$mcolor>
						<td><b><hr><font color=$mcolor> $mlorr sebelum pajak </td>
						<td align=right><b><hr><font color=$mcolor> $mctot </td>
					</tr>
					<tr>
						<td>&nbsp;</td>
		...
		echo "
					<tr>
						<td>$rrow[rekid] $rrow[reknama] </td>
						<td align=right> $mjum </td>
					</tr>
		";
		...
		if ($mtotalall<0)
		{
			$mlorr='Rugi';
			$mcolor='red';
			$mctot='('.number_format(abs($mtotalall),0,'.',',').')';
			$mctot2=number_format(abs($mtotal),0,'.',',');
		}
		else
		{
			$mlorr='Laba';
			$mcolor='blue';
			$mctot=number_format($mtotalall,0,'.',',');
			$mctot2=number_format(abs($mtotal),0,'.',',');
		}
	}
	...
	echo "
					<tr>
						<td><b><hr> Total HPP </td>
						<td align=right><b><hr> $mtotal </td>
					</tr>
					<tr>
						<td><b><hr><font color=$mcolor> Total $mlorr </td>
						<td align=right><b><hr><font color=$mcolor> $mctot </td>
					</tr>
					<tr>
						<td>&nbsp;</td>
	...
	from setrek b
	left join setneraca c on b.nrcid=c.nrcid
	left join setklas d on c.clid=d.clid
	where d.posisi in ('D','B')
	order by b.rekid
	";
	...
	while ($rrow=mysql_fetch_assoc($mjumm1))
	{
		$query2665=executerow("select sum(kredit-debet) jumlah from bkbesar where tgl <= '$mtgl2' and rekid='$rrow[rekid]'");
		$mjj=mysql_fetch_assoc ($query2665);
...
if ($mLapo=='mlaplunaspiutang')
{
	$link=open_connection();
	$sqlstr="
	select a.*, b.lgnnama, c.nid jualid, c.tgl tgljual, c.debet
	from bkbesar a
	left join setlgn b on a.bpid=b.lgnid 
	left join
	(
		select bpid, nid, tgl, debet
		from bkbesar
		where rekid='10210' and debet<>0
	) c on a.nid2=c.nid and a.bpid=c.bpid
	...
	while ($row = mysql_fetch_assoc ($result))
	{
		$query2828=executerow("select sum(kredit) kredit from bkbesar where rekid='10210' and trans='TRANSRETURJUAL' and nid2='$row[jualid]' ");
		$rowr=mysql_fetch_assoc ($query2828);
		
		$query2831=executerow("select sum(kredit) kredit from bkbesar where rekid='10210' and trans in ('TRANSLUNASPIUTANG','TRANSKASIR') and nid2='$row[jualid]' and nid<>'$row[nid]'");
		$rowp=mysql_fetch_assoc ($query2831);
		
		$msaldo=$row[debet]-$rowr[kredit]-$rowp[kredit];
		$query2835=executerow("select sum(kredit) potongan from bkbesar where rekid='10210' and nid='$row[nid]' and ket like '%Potongan%' and nid2='$row[jualid]' ");
		$row2=mysql_fetch_assoc ($query2835);
		
		$query2838=executerow("select sum(debet) kupon from bkbesar where rekid='20310' and nid='$row[nid]' and nid2='$row[jualid]' ");
		$row3=mysql_fetch_assoc ($query2838);
		
		$query2841=executerow("select sum(debet) giro from bkbesar where rekid='10220' and nid='$row[nid]'");
		$row4=mysql_fetch_assoc ($query2841);
		
		$query2844=executerow("select sum(debet) transfer from bkbesar where rekid like '101%' and nid='$row[nid]'");
		$row5=mysql_fetch_assoc ($query2844);
		
		$query2847=executerow("select sum(debet) tunai from bkbesar where rekid='10010' and nid='$row[nid]'");
		$row6=mysql_fetch_assoc ($query2847);
...
if ($mLapo=='mlaplunashutang')
{
	$link=open_connection();
	$sqlstr="
	select a.*, b.supnama, c.nid jualid, c.tgl tgljual, c.kredit
	from bkbesar a
	left join setsup b on a.bpid=b.supid 
	left join
	(
		select bpid, nid, tgl, kredit
		from bkbesar
		where rekid='20010' and kredit<>0
	) c on a.nid2=c.nid and a.bpid=c.bpid
	...
	while ($row = mysql_fetch_assoc ($result))
	{
		$query2968=executerow("select sum(debet) debet from bkbesar where rekid='20010' and trans='TRANSRETURBELI' and nid2='$row[jualid]' and bpid='$row[bpid]' ");
		$rowr=mysql_fetch_assoc ($query2968);
		
		$query2971=executerow("select sum(debet) debet from bkbesar where rekid='20010' and trans='TRANSLUNASHUTANG' and nid2='$row[jualid]' and tgl<'$row[tgl]' and bpid='$row[bpid]' ");
		$rowp=mysql_fetch_assoc ($query2971);
		
		$msaldo=$row[kredit]-$rowr[debet]-$rowp[debet];
		$query2976=executerow("select sum(debet) potongan from bkbesar where rekid='20010' and nid='$row[nid]' and ket like '%Potongan%' and nid2='$row[jualid]' and bpid='$row[bpid]' ");
		$row2=mysql_fetch_assoc ($query2976);
		
		$query2978=executerow("select sum(kredit) kupon from bkbesar where rekid='20310' and nid='$row[nid]' and nid2='$row[jualid]' and bpid='$row[bpid]' ");
		$row3=mysql_fetch_assoc ($query2978);
		
		$query2981=executerow("select sum(kredit) giro from bkbesar where rekid='10220' and nid='$row[nid]' and bpid='$row[bpid]' ");
		$row4=mysql_fetch_assoc ($query2981);
		
		$query2984=executerow("select sum(kredit) transfer from bkbesar where rekid like '101%' and nid='$row[nid]' and bpid='$row[bpid]' ");
		$row5=mysql_fetch_assoc ($query2984);
		
		$query2987=executerow("select sum(debet) bayar from bkbesar where rekid='20010' and debet<>0 and nid='$row[nid]' and nid2='$row[jualid]' and ket like 'Pelunasan%' and bpid='$row[bpid]' ");
		$row6=mysql_fetch_assoc ($query2987);
		...
		if ($mnidd!=$row[nid])
		{
			$query3027=executerow("select count(*) cnnt from bkbesar where nid='$row[nid]' and nid2<>'' and debet<>0 and rekid='20010' ");
			$moo=mysql_fetch_assoc ($query3027);
			
			$query3030=executerow("select sum(kredit) giro from bkbesar where rekid='20110' and nid='$row[nid]' and bpid='$row[bpid]' ");
			$row4=mysql_fetch_assoc ($query3030);
			
			$query3033=executerow("select sum(kredit) transfer from bkbesar where rekid like '101%' and nid='$row[nid]' and bpid='$row[bpid]' ");
			$row5=mysql_fetch_assoc ($query3033);
			
			$query3036=executerow("select sum(kredit) tunai from bkbesar where rekid='10010' and kredit<>0 and nid='$row[nid]' ");
			$row6=mysql_fetch_assoc ($query3036);
...
if ($mLapo=='mlapjualnota')
{
	...
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and a.lgnid like '!!$mlgnid!!' and a.sales like '!!$mkarid!!'
	order by a.nid, a.tgl
	";

	$query3277=executerow("select * from setkaryawan where karid='$mkarid'");
	$saa=mysql_fetch_assoc ($query3277);
	...
	{
		$mrr->total = $mrr->total+$mrr->ongkos;
		echo "
		<tr valign= height=25>
			<td>$nn</td>
			<td>$mrr->lgnid</td>
			...
			<td align=right>".number_format($mrr->total,0,'.',',')."</td> 
			<td align=right>".number_format($mrr->tunai,0,'.',',')."</td> 
			<td align=right>".number_format($mrr->total-$mrr->tunai,0,'.',',')."</td> 
			<td align=right>".number_format($mrr->retur,0,'.',',')."</td> 
			<td align=right>".number_format($mrr->lunas,0,'.',',')."</td> 
			<td align=right>".number_format($mrr->total-$mrr->retur-$mrr->lunas-$mrr->tunai,0,'.',',')."</td>
		</tr>
		";

		$totot=$totot+$mrr->total;
		$tottunai=$tottunai+$mrr->tunai;
		$totPIUTANG=$totPIUTANG+$mrr->total-$mrr->tunai;
		$totretur=$totretur+$mrr->retur;
		$totlunas=$totlunas+$mrr->lunas;
		$totsisa=$totsisa+($mrr->total-$mrr->retur-$mrr->lunas-$mrr->tunai);
	...
	echo '
		...
		<tr style="vertical-align:bottom">
			<td colspan="6" style="text-align:right">Komisi&nbsp;</td>
			<td style="text-align:right;font-weight:bold">'.number_format($totkomisi, 0, ".", ",").'</td>
			<td colspan="5">&nbsp;</td>
		</tr>
	</table>
	';
...
if ($mLapo=='mlapjualnota2')
{
	...
	$sqlstr="
	select a.*, b.lgnnama, date_format(tgl, '%d-%m-%Y') tglc, date_format(tgl, '%d-%m-%Y') tgljtc, date_format(tgl, '') tgljth, c.retur, d.lunas
	from transkasir1 a
	left join setlgn b on a.lgnid=b.lgnid
	...
	left join (select nid2, sum(kredit) lunas
		from bkbesar
		where rekid='10210' and trans='TRANSLUNASPIUTANG'
		group by nid2) d on a.nid=d.nid2
	where tgl between str_to_date('".$mtg1."', '%d-%c-%Y') and str_to_date('".$mtg2."', '%d-%c-%Y') and a.kasir like '!!".$mkasir."!!' and a.lgnid like '!!".$lgnid."!!' and ".$ttk."
	order by a.nid, a.tgl
	";
