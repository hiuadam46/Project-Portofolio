...
<html><head>
	...
	$Pcs2(document).ready(function(){
	  ...
	  $Pcs2(document).keydown(function(){
        ...
        $Pcs2("#tambah").click(function() {
			...
			dialogisopen=true;
			fsetstok.reset();
			datas=taptabel("setlok","lokid","1=1 order by lokid desc limit 1");
			data=datas.lokid;
			
			if (data!=undefined)
			{
			mmlokid=data.substr(2,5);
			//alert(mmlokid);
			mmlokid=parseFloat(mmlokid);
			//alert(mmlokid);
			mmlokid=mmlokid+1;
			mmlokid=''+mmlokid
			mmlokid='L'+padl(mmlokid,'0',3);
			}
			else
			{
			mmlokid='L001';
			}
			fsetstok.mlokid.value=mmlokid;

		fsetstok.mloknama.focus();fsetstok.mloknama.select();
		$Pcs2("#msatnama").click()
		})
        ...
        $Pcs2("#buka").click(function() {
			...
			//fsetstok.mlokid.enabled=true;

			$Pcs2("#kotakdialog").dialog('open');
			dialogisopen=true;

			fsetstok.mlokid.value=isi;
			$Pcs2.ajax({
			type:"POST",
			url:"phpexec.php",    
			data: "func=EXEC&field=*&tabel=setlok&kondisi=(lokid='"+isi+"')",
			dataType:'json',
			success: function(data){

	maj=document.getElementById("mlokid")
	...
	for (var mii=1;mii<30;mii++)
	{
		...
		if (mtp=='fieldna2')
		{
			...
			fsetstok.mlokid.focus();fsetstok.mlokid.select();
	    });
	...
	maj=document.getElementById("mlokid")
	msu=$Pcs2("#mlokid").val()
	execajaxas("delete from setlok where lokid='"+msu+"'")
	execajaxas("insert into setlok (lokid) values('"+msu+"')")
	...
	for (var mii=1;mii<30;mii++)
	{
		...
		if (mtp=='fieldna')
		{
			...
			execajaxa("update setlok set "+mfieldna+"='"+maj.value+"' where lokid='"+msu+"'")
			
		}
		if (mtp=='fieldna2')
		{
			mfieldna=maj.id.substr(1,100)
			execajaxa("update setlok set "+mfieldna+"='"+toval(maj.value)+"' where lokid='"+msu+"'")
	...
	return;	
        ...
        $Pcs2("#btnhapus").click(function() {
			...
			mlokid=fsetstok.mlokid.value;
			$Pcs2.ajax({
			type:"POST",
			url:"phpexec.php",
			data: "func=EXECUTE&comm=delete from setlok where lokid='"+mlokid+"'",
			//dataType:'json',
			success: function(data){
			if(data==1)
			...
			execajaxa("delete from bkbesar where bpid='"+mlokid+"'")
        ...
        $Pcs2("#btnkontak").click(function() {
			...
			$Pcs2("#kotakdialog2").dialog('option', 'title', 'Kontak lok '+$Pcs2("#msatnama").val());
            $Pcs2("#kotakdialog2").dialog('open');
			dialogisopen=true;
		})
		...
		//mfilt=mfilt.trim();
		//alert(mfilt);
		//tableGrid1.request="msql=select * from setlok where (concat(lokid,satnama,rekid) like '!!"+mfilt+"!!') order by lokid"
		//tableGrid1.refresh()
		//});		
		...
		$Pcs2("#tcetak").click(function() {
			window.open("isilaporan.php?mLapo=mSetup&mtabelnya=setlok&detitle=Master lok<br>")
		})
...
//    SyntaxHighlighter.all();
   ...
   var mws=screen.width
	...
	url: "item.php",
	request: "msql=select *,true as edit from setlok where (1=1) order by lokid"
    };
	...
	function proses(){
		...
		$Pcs("#buka").click();
		//alert(isi);
		//window.location='setlok.php?mlokid='+isi;
...
?>	
	...
	<div id="kotakdialog" title="Setup Lokasi">	
	...
	WHERE TABLE_SCHEMA='siamart_2jaya_data' and TABLE_NAME='setlok' and COLUMN_COMMENT <>''
