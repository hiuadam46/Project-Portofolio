...
$mwd4='10 disabled';
echo("
			<th rowspan=1> Retur </th>
");
$mwd5='10 disabled';
echo("
			<th rowspan=1> Lunas </th>
");
$mwd6='10 disabled';
echo("
...
$query83=executerow($query);
$mqwee=mysql_fetch_assoc ($query83);

if ($mqwee[nid]=='')
{
	$query="
	select nid, date_format(tgl, '%d-%m-%Y') tgl, format(debet, 0) debet, format(ifnull(b.kredit, 0), 0) lunas, format(ifnull(c.kredit, 0), 0) retur, format(ifnull(d.kredit, 0), 0) bayar, format(debet-ifnull(b.kredit, 0)-ifnull(c.kredit, 0)-ifnull(d.kredit, 0), 0) saldo
	from bkbesar a
	left join
	(
		select nid2, sum(kredit) kredit
		from bkbesar
		where rekid='10210' and trans<>'TRANSRETURJUAL' and nid<'$mnid' and bpid='$mlgnid'
		group by nid2
	) b on a.nid=b.nid2 
	left join
	(
		select nid2, sum(kredit) kredit
		from bkbesar
		where rekid='10210' and trans='TRANSRETURJUAL' and bpid='$mlgnid'
		group by nid2
	) c on a.nid=c.nid2
	left join
	(
	...
	where rekid='10210' and a.debet>0 and a.bpid='$mlgnid' and (a.debet-ifnull(b.kredit,0)-ifnull(c.kredit, 0)-ifnull(d.kredit, 0)) >0
	";
}
else
{
	$query="
	select nid, date_format(tgl, '%d-%m-%Y') tgl, format(debet, 0) debet, format(ifnull(b.kredit, 0), 0) lunas, format(ifnull(c.kredit, 0), 0) retur, format(ifnull(d.kredit, 0), 0) bayar, format(debet-ifnull(b.kredit, 0)-ifnull(c.kredit, 0), 0) saldo, format(debet-ifnull(b.kredit,0)-ifnull(c.kredit, 0)-ifnull(d.kredit, 0), 0) sisa
	from bkbesar a
	left join
	(
		select nid2, sum(kredit) kredit
	...
	where rekid='10210' and a.debet>0 and a.bpid='$mlgnid' and a.nid in
	(
		select nid2
		from bkbesar
		where nid='$mnid'
...
while ($rows=mysql_fetch_assoc($rrw))
{
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text_$mnom' $midname size=$mwd2 value='".$rows[nid]."' $nstt onblur='ambilstok(this)'></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text_$mnom' $midname size=$mwd3 value='".$rows[tgl]."' $nstt></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd4 value='".$rows[debet]."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd5 value='".$rows[retur]."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd6 value='".$rows[lunas]."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd7 value='".$rows[saldo]."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd8 value='".$rows[bayar]."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd9 value='".$rows[sisa]."' $nstt2></td>
	";

	$query222=executerow("select date_format(tgljt,'%d-%m-%Y') tgljt from transjual1 where nid='$rows[nid]'");
	$mgj=mysql_fetch_assoc ($query222);
	
	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd10 value='".$mgj[tgljt]."' $nstt2></td>
	";
