...
for ($ggm=0;$ggm<$jum;$ggm++)
{
	...
	echo("
			<th rowspan=1> <input type=button value='$moor' id='$mnam2' $msty onclick=focuskan() onmousedown=urutkan(this)> $mnam <input type=button value=' ' $msty > </th>
	");
}
echo("
		</tr>
...
for ($ggm=0;$ggm<$jum;$ggm++)
{
	...
	echo("
			<th><input size=$mpan type='text' disabled $msty></th>
	");
}
...
while ($rows=mysql_fetch_array($rrw))
{
	...
	for ($ggm=0;$ggm<$jum;$ggm++)
	{
		...
		{
			$msto=$rows['stoid'];
			$query135=executerow("select right(concat(truncate(sum(qtyin-qtyout)/isi, 0), ' ', satuan1, ' ', truncate(mod(sum(qtyin-qtyout), isi), 0), ' ', satuan2), 20) saldo from bkbesar b left join setstok a on b.bpid=a.stoid and a.stoid='".$msto."' where bpid2='".$misi."' and bpid='".$msto."'");
			$nnnt=mysql_fetch_assoc ($query135);
		...
		$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
		echo "
			<td><input type='text_$mnom' $midname size=$mpan value='$misi' $maliggn readonly></td>
		";
	}
