...
");
$mstok=mysql_fetch_assoc($query22);

$mmstoid=$mstok[stoid];
$mbarr=$mstok[barcode];
$meed='Edit';

$query29=executerow("select nid from bkbesar where bpid='$_GET[mstoid]' and rekid='10310' limit 0, 1");
$mtra=mysql_fetch_assoc($query29);
...
if ($mmstoid=='')
{
	...
	");
	$mstokx=mysql_fetch_assoc($query40);
...
<form id="fform">
	...
	<table border="0" style="border-color: aqua" width="100%">
		<tr>
			...
			Isi :
			<input id="misi" name="misi" size="5" style="max-width: 24pt" value="<?php echo $mstok[isi] ?>"><input id="msatuan2" disabled name="msatuan2" size="5" type="text" value="<?php echo $mstok[satuan2] ?>"></td>
		</tr>
		<tr>
		...
		<tr>
			<td align="left">Hrg. Kredit&gt; Partai </td>
			<td colspan="3">:
		...
		<tr>
			<td align="left">-------------&gt; Ecer </td>
			<td colspan="3">:
			<input id="mhrgjual5" name="mhrgjual5" size="10" style="text-align: right" type="text" value="<?php echo number_format($mstok[hrgjual5],0,'.',',') ?>"></td>
		</tr>
		<tr>
			<td align="left">Hrg. Jual Eceran </td>
			<td colspan="3">:
