...
function transform($mcoba)
{
	...
	if (!empty($mhas1)){$mteng='.';}else{$mteng='';};
...
function currencytoWord($number)
{
	...
	{
		$temp = currencytoWord($x - 10). " belas";
	} else if ($x <100)
	{
		$temp = currencytoWord($x/10)." puluh". currencytoWord($x % 10);
	} else if ($x <200)
	{
		$temp = " seratus" . currencytoWord($x - 100);
	} else if ($x <1000)
	{
		$temp = currencytoWord($x/100) . " ratus" . currencytoWord($x % 100);
	} else if ($x <2000)
	{
		$temp = " seribu" . currencytoWord($x - 1000);
	} else if ($x <1000000)
	{
		$temp = currencytoWord($x/1000) . " ribu" . currencytoWord($x % 1000);
	} else if ($x <1000000000)
	{
		$temp = currencytoWord($x/1000000) . " juta" . currencytoWord($x % 1000000);
	} else if ($x <1000000000000)
	{
		$temp = currencytoWord($x/1000000000) . " milyar" . currencytoWord(fmod($x,1000000000));
	} else if ($x <1000000000000000)
	{
		$temp = currencytoWord($x/1000000000000) . " trilyun" . currencytoWord(fmod($x,1000000000000));
	}
	return $temp;
}
...
{
	$query225=executerow("select rekid,hrgbeli,isi*isi2 isi from setstok a left join setgrp b on a.grpid=b.grpid where a.stoid='$mstoid'");
	$sqr=mysql_fetch_assoc ($query225);
	...
	if ($mqtyout>0)
	{
		...
		{
			/* $sqlstr="
			select a.bpid AS stoid, a.bpid2 AS lokid, a.nid AS nid, a.tgl AS tgl, (sum(a.qtyin)-sum(ifnull(b.qtyout,0))) AS saldoqty, round(a.debet/a.qtyin,0) AS hpp
			from
			(
				bkbesar a
				left join
				(
					select nid2, bpid, sum(qtyout) qtyout
					from bkbesar
					where bpid='$mstoid' and bpid2='$mlokid'
					group by nid2, bpid
				) b on ((a.bpid = b.bpid) and (a.nid = b.nid2) and (b.qtyout <> 0))
			) 
			...
			$sqlstr="
			select a.nid AS nid, a.tgl AS tgl, a.debet/a.qtyin hpp, a.qtyin-ifnull(b.qtyout, 0) AS saldoqty, a.qtyin, b.qtyout
			from
			(
				select nid, tgl, qtyin, debet
				from bkbesar a
				where a.rekid like '103%' and a.qtyin > 0 and a.bpid='$mstoid' and a.bpid2='$mlokid'
			) a
			left join
			...
			$query274=executerow($sqlstr);
			$maa=mysql_fetch_assoc ($query274);
			...
			$sqlstr="
			insert into bkbesar(rekid, bpid, bpid2, tgl, nid, kredit, qtyout, trans, tgl2, nid2, ket)
	...
	if ($mqtyin>0)
	{
		...
		$sqlstr="
		insert into bkbesar(rekid, bpid, bpid2, tgl, nid, debet, qtyin, trans, ket)
...
{
	$query400=executerow("select * from setstok where stoid='$mstoid'");
	$mrww=mysql_fetch_assoc ($query400);
