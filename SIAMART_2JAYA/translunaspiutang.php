  ...
  $Pcs2(document).ready(function(){
	...
	$Pcs2("#mnid").blur(function() {
		...
		if (mgg.nid!=undefined)
		{
			...
			mgg2=taptabel("bkbesar","ifnull(debet,0) debet","nid='"+mnid+"' and rekid='10010'")
			$Pcs2("#mkas").val(tra(mgg2.debet))

			mgg3=taptabel("bkbesar","ifnull(rekid,'') rekid,ifnull(debet,0) debet","nid='"+mnid+"' and rekid like '101%' and ket not like 'Pencairan%'")
			$Pcs2("#mtransfer").val(tra(mgg3.debet))
			$Pcs2("#mrekid").val(mgg3.rekid)

			mgg4=taptabel("bkbesar","ifnull(sum(debet),0) debet","nid='"+mnid+"' and rekid='10220'")
	...
	$Pcs2("#lookmnid").click(function() {
		mcomm="Select nid Nota,date_format(tgl,'%d-%m-%Y') Tgl,left(concat(a.bpid,'-',lgnnama),30) pelanggan,left(FORMAT(sum(kredit),0),12) rightTotal from bkbesar a left join setlgn b on a.bpid=b.lgnid where trans='TRANSLUNASPIUTANG' and a.kredit>0 and ket not like '%Pencairan%' group by a.nid,a.tgl,a.bpid,b.lgnnama "
		mordd="Nota"
...
////Cek giro			
		...
		success : function(data){
			...
			if (mtglx<=mtgljt)
			{
			abgg=taptabel("transjual1 a left join setkaryawan b on a.sales=b.karid","b.gaji,a.sales,b.karnama","a.nid='"+mbeliid+"'")
			komisi=toval(abgg.gaji)*mbayar*0.01

			msqql1="insert into bkbesar(rekid ,bpid,tgl,nid,kredit,trans,ket,nid2) "			
			msqql2="values('40040','"+abgg.sales+"',"+mtgl+",'"+mnid+"',"+komisi+",'TRANSLUNASPIUTANG','Komisi atas No. "+mbeliid+"','"+mbeliid+"')"
			execajaxa(msqql1+msqql2)

			msqql1="insert into bkbesar(rekid ,bpid,tgl,nid,debet,trans,ket,nid2) "			
			msqql2="values('20120','"+abgg.sales+"',"+mtgl+",'"+mnid+"',"+komisi+",'TRANSLUNASPIUTANG','Pelunasan No. "+mbeliid+"','"+mbeliid+"')"
			execajaxa(msqql1+msqql2)
...
////giro
			...
			msqql2="values('10220','"+mlgnid+"','"+mrekid+"','"+mtglgiro+"','"+mnid+"',"+mjumlah+",'TRANSLUNASPIUTANG','Pencairan Giro No. "+mnid+"','"+mgiroid+"','"+mtglgiro+"')"
			execajaxa(msqql1+msqql2)
