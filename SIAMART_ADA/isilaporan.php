...
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_ada_data' and TABLE_NAME='$mtabelnya'
";
...
{
$msql="SELECT COLUMN_NAME kolom,DATA_TYPE tipe,NUMERIC_PRECISION panjang1,CHARACTER_MAXIMUM_LENGTH panjang2,COLUMN_COMMENT koment
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_ada_data' and TABLE_NAME='$mtabelnya' and COLUMN_NAME in ($mkol)
";
}
	...
	while ($mrr=mysql_fetch_object($maa))
	{
		...
		// 15JULI2020
				...
				while ($msx=mysql_fetch_object($mss))
				{
					...
					$cdiskoncek = str_replace(',', '', $msx->cdiskon);
					if(is_numeric($cdiskoncek)){
						$diskonnya = $msx->hrgsat-$cdiskoncek;
					}else{
						$diskonpersen = str_replace('%', '', $msx->cdiskon);
						$diskonnya = $msx->hrgsat-($msx->hrgsat*($msx->cdiskon/100));
					}
					//15JUL2020
					echo '
					<tr>
					...
					<td>&nbsp;'.$msattu.'</td>
					<td align=right>'.number_format($diskonnya,0,',','.').'</td>
...
if ($mLapo=='mlapjualnota')
{
	...
	{
		echo("
		ADA SENTOSA STATIONERY (PPN)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NPWP : 06.628.748.3-645.000
		<br>
		Laporan Penjualan Per Nota
		<br>Tgl. $mtg1 s.d. $mtg2 $mtgt
		<br><br>
	...
	{
		echo("
		ADA SENTOSA STATIONERY (NonPPN)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NPWP : 06.628.748.3-645.000
		<br>
		Laporan Penjualan Per Nota
		<br>Tgl. $mtg1 s.d. $mtg2 $mtgt
		<br><br>
...
<td colspan=5></td>
<td align='center'><strong>DPP</strong></td>
<td align=right><b>".number_format($totot-$ppnlast,0,',','.')."</td>
<td colspan=5></td>
</tr>
";
...
<td colspan=5></td>
<td align='center'><strong>PPN</strong></td>
<td align=right><b>".number_format($ppnlast,0,',','.')."</td>
<td colspan=5></td>
</tr>
";
}
