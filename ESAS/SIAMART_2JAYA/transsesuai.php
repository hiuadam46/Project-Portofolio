...
$Pcs2(document).ready(function()
{
	...
	$Pcs2("#mlokid1").attr('disabled', false);
		...
		for (ma=1;ma<100;ma++)
		{
			...
			if (mstoid!='')
			{
				...
				if (mqqty>0)
				{
					...
					mjmlhrg=savstofifo(mlokid1, mstoid, mnid, mtglx, 0, 0, 0, mqqty, 'TRANSSESUAI', 'Penyesuaian stok '+mstoid)
				...
				else
				{
					...
					mjmlhrg=savstofifo(mlokid1, mstoid, mnid, mtglx, (mqqty*(mhrgbeli/mmisi)), 0, mqqty, 0, 'TRANSSESUAI', 'Penyesuaian stok')
	...
	$Pcs2("#mnid").blur(function()
	{
		...
		if (mnid!=undefined)
		{
			...
			$Pcs2('#btnsimpan').attr('disabled',true);
		}
		else
		{
...
function baru()
{
	...
	$Pcs2('#btnsimpan').attr('disabled',false);
}
</script>
</head>
...
?></td>
		...
		<tr>
			<td>No.</td>
			<td>: <input id="mnid" size="10" type="text">
			<input id="lookmnid" type="button" value="F5"> </td>
			<td></td>
			<td></td>
			<td></td>
...
<div id="kotakdialog2" title="Setup Pelanggan">
	<center><iframe id="framehrg" height="400" style="top: 30px" width="750">
	</iframe></center></div>
