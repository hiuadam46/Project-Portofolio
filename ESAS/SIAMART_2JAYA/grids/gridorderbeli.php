...
$query="
select a.stoid, FORMAT(a.qty, 0) qty, FORMAT(a.unit, 0) unit, FORMAT(a.extra, 0) extra, FORMAT(a.extraunit, 0) extraunit, FORMAT(hrgsat, 0) hrgsat, FORMAT(diskonp, 2) diskon, FORMAT(a.jmlhrg,0) jmlhrg
from transbeli2 a
right join setstok b on a.stoid=b.stoid
where a.nid='$mnid' and b.supid='$msupid'
order by a.nomor
";
...
while ($rows=mysql_fetch_assoc($rrw))
{
	...
	$mtoid=$rows[stoid];
	$query130=executerow("select stonama,satuan1,satuan2,isi from setstok where stoid='$mtoid'");
	$nng=mysql_fetch_assoc ($query130);
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd13 value='".$rows[jmlhrg]."' $nstt2><input type=hidden id='misi_$mnom' name='misi_$mnom' value='$nng[isi]'></td>
	";
...
for ($mgg=$mnom;$mgg<100;$mgg++)
{
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd13 value='".$rows[jmlhrg]."' $nstt2><input type=hidden id='misi_$mnom' name='misi_$mnom'></td>
	";
...
<input type=text size=5 id='tabjumlah' hidden value='$mjum' readonly style='text-align:center'>
	<table border=1 cellpadding=0 cellspacing=0 style='border-collapse:collapse'>
		<tr>
			<td bgcolor='white' rowspan=1 colspan=10 align=center>&nbsp;<font color=red><span id='spanket'></span></td>
			<td bgcolor='white' colspan=2 align=right>Total&nbsp;&nbsp;</td>
			<td bgcolor='white'><input size=$mwd13 type='text' $nstt2 readonly id='mtotal'></td>
		</tr>
