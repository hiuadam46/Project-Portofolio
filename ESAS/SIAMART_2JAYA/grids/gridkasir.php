...
$query='
select a.stoid, FORMAT(a.qty, 2) qty, FORMAT(a.hrgsat, 0) hrgsat, cdiskon diskon, FORMAT(a.jmlhrg, 0) jmlhrg, a.satuan, a.isi, b.stonama
from transkasir2 a
...
while ($rows=mysql_fetch_assoc($rrw))
{
	...
	$mjrow=$mjrow+1;
	$midname="id='15".'_'."$mnom' name='15".'_'."$mnom'";
	echo "
	...
	$mjrow=$mjrow+1;
	$midname="id='20".'_'."$mnom' name='20".'_'."$mnom'";
	echo "
...
for ($mgg=$mnom;$mgg<100;$mgg++)
{
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd4 value='".$rows[satuan1]."' $nstt></td>
	";
