...
$query78=executerow($query);
$mqwee=mysql_fetch_assoc ($query78);

if ($mqwee[nid]=='')
{
	$query="
	select nid, date_format(tgl, '%d-%m-%Y') tgl, format(kredit, 0) kredit, format(ifnull(b.debet, 0),0) lunas, format(ifnull(c.debet, 0), 0) retur, format(ifnull(d.debet, 0), 0) bayar, format(kredit-ifnull(b.debet, 0)-ifnull(c.debet, 0)-ifnull(d.debet, 0), 0) saldo
	from bkbesar a
	left join
	(
		select nid2, sum(debet) debet
		from bkbesar
		where rekid='20010' and trans<>'TRANSRETURBELI' and nid<'$mnid' and bpid='$msupid'
		group by nid2
	) b on a.nid=b.nid2 
	left join
	(
		select nid2, sum(debet) debet
		from bkbesar
		where rekid='20010' and trans='TRANSRETURBELI' and bpid='$msupid'
		group by nid2
	) c on a.nid=c.nid2
	left join
	(
	...
	where rekid='20010' and a.kredit>0 and a.bpid='$msupid' and (a.kredit-ifnull(b.debet,0)-ifnull(c.debet, 0)-ifnull(d.debet, 0)) >0
	";
}
else
{
	$query="
	select nid, date_format(tgl, '%d-%m-%Y') tgl, format(kredit, 0) kredit, format(ifnull(b.debet, 0),0) lunas, format(ifnull(c.debet, 0), 0) retur, format(ifnull(d.debet, 0), 0) bayar, format(kredit-ifnull(b.debet, 0)-ifnull(c.debet, 0), 0) saldo, format(kredit-ifnull(b.debet,0)-ifnull(c.debet, 0)-ifnull(d.debet, 0), 0) sisa
	from bkbesar a
	left join
	(
		select nid2, sum(debet) debet
	...
	where rekid='20010' and a.kredit>0 and a.bpid='$msupid' and a.nid in
	(
		select nid2
		from bkbesar
		where nid='$mnid'
...
while ($rows=mysql_fetch_assoc($rrw))
{
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd4 value='".$rows[kredit]."' $nstt2></td>
	";
