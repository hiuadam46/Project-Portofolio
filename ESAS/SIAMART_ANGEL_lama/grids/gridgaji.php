...
echo("<th rowspan=1> Kode </th>");$mwd2='6 readonly';
echo("<th rowspan=1> Nama </th>");$mwd3='30 readonly';
echo("<th rowspan=1> Bagian </th>");$mwd4='8 readonly';
echo("<th rowspan=1> Grup </th>");$mwd5='20 readonly';
echo("<th rowspan=1> Jml.Bor</th>");$mwd6='10 readonly';
echo("<th rowspan=1> Jml.Tamb</th>");$mwd7='10';
echo("<th rowspan=1> Jam.Har</th>");$mwd8='10 readonly';
echo("<th rowspan=1> Jam.Lemb</th>");$mwd9='10 ';
echo("<th rowspan=1> Pot.Jam </th>");$mwd10='10';
echo("</tr>");
echo("
<tr>
...
if ($mstatus=='')
{
$query="select a.karid,a.karnama,a.grup,a.bagian,a.gaji,FORMAT(ifnull(b.kredit,0),0) borongan,jmlborongan 
from setkaryawan a 
left join (select bpid,count(*) jmlborongan,sum(kredit) kredit from bkbesar where rekid='20110' and tgl='$mtgl' and kredit>0 group by bpid) b on a.karid=b.bpid
where true order by karid";
...
while ($rows=mysql_fetch_assoc($rrw))
	{
		...
		value='".$rows[karid]."' $nstt onblur='ambilstok(this)'></td>
		";
		...
		value='".$rows[jmlborongan]."' $nstt2 ></td>
		";

		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td><input type='text' $midname size=$mwd7
		value='".$rows[jmltambahan]."' $nstt2 ></td>
		";

		$mrrr1=execute("select time_to_sec(jam) jam from absensi where karid='".$rows[karid]."' and tgl='$mtgl' order by jam limit 0,1");
		$mrrr2=execute("select time_to_sec(jam) jam from absensi where karid='".$rows[karid]."' and tgl='$mtgl' order by jam desc limit 0,1");
		...
		value='$mlembur1.$mlembur2' $nstt2 ></td>
		";

		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td><input type='text' $midname size=$mwd10
		value='".$rows[jampotongan]."' $nstt2 ></td>
		";
...
{
$query="select a.*,b.karnama,b.grup,b.bagian from transgaji a left join setkaryawan b on a.karid=b.karid where tgl='$mtgl' order by a.karid";
...
while ($rows=mysql_fetch_assoc($rrw))
	{
		...
		echo "	
		<td><input type='text' $midname size=$mwd6
		value='".$rows[borongan]."' $nstt2 ></td>
		";

		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td><input type='text' $midname size=$mwd7
		value='".$rows[tambahan]."' $nstt2 ></td>
		";
		...
		echo "
		<td><input type='text' $midname size=$mwd9
		value=".$rows[lembur]." $nstt2 ></td>
		";

		$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom'  name='$mjrow".'_'."$mnom'";		
		echo "	
		<td><input type='text' $midname size=$mwd10
		value='".$rows[potongan]."' $nstt2 ></td>
		";
