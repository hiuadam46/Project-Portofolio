...
<html><head>
	...
	function buatlaporan(ngeprint, excel)
	{
			...
			mrute=$Pcs2("#msupid").val()
			mjnsbayar=$Pcs2("#mjnsbayar").val()
			if(!excel)
			{
				if (!ngeprint)
				{
				$Pcs2("#framelap").attr("src","isilaporan.php?mLapo=mlaplunashutang&mtg1="+mtgl1+"&mtg2="+mtgl2+"&mjnsbayar="+mjnsbayar+"&mrute="+mrute)
				}
				else
				{
				window.open("isilaporan.php?mLapo=mlaplunashutang&mtg1="+mtgl1+"&mtg2="+mtgl2+"&mrute="+mrute+"&mjnsbayar="+mjnsbayar+"&isprint=YY")
				}
			}
			else
			{
				window.open("isilaporan.php?mLapo=mlaplunashutang&excel=true&mtg1="+mtgl1+"&mtg2="+mtgl2+"&mrute="+mrute+"&mjnsbayar="+mjnsbayar+"&isprint=YY")
			}
	}
    </script>
    <script type="text/javascript">
...
?>	
	...
	<tr>
		<td>Tgl.&nbsp;&emsp;&emsp;&nbsp; 
		<input type=text id=mtgl1 size=10 readonly> S.d. <input type=text id=mtgl2 size=10 readonly></td>
	...
	<tr>
	<td>Supplier 
			: <input id="msupid" size="5" type="text">
			<input id="lookmsupid" type="button" value="F5">
			<input id="msupnama" disabled size="45" type="text">
		Cara Bayar :
			<select id="mjnsbayar"> 
