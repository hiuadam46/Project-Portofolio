...
<head>
   ...
   $Pcs2("#kotakdialog2").dialog({
	...
	execajaxas("delete from transkaskeluar where nid='"+mnid+"'")
	...
	for (ma=1;ma<mvv;ma++)
		{
			...
			{
			//alert(mnomor)
			execajaxa("insert into transkaskeluar(nid,tgl,nomor,rekid,jumlah,ket) values('"+mnid+"',"+mtgl+",'"+mnomor+"','"+mrekid+"','"+mjumlah+"','"+mket+"')")

			msqql="insert into bkbesar(rekid ,bpid ,tgl,nid,debet,trans,ket) values('"+mrekid+"','"+mbpid+"',"+mtgl+",'"+mnid+"',"+mjumlah+",'transkaskeluar','"+mket+"')"
			execajaxa(msqql)
			
			msqql="insert into bkbesar(rekid ,bpid ,tgl,nid,kredit,trans,ket) values('"+mrekida+"','',"+mtgl+",'"+mnid+"',"+mjumlah+",'transkaskeluar','"+mket+"')"
			execajaxa(msqql)
	...
	function baru()
	{
		...
		datas=taptabel("bkbesar","nid","trans='TRANSKASKELUAR' order by nid desc limit 0,1")
		data=datas.nid;
			...
			execajaxa("delete from transkaskeluar where nid='"+mmmerkid+"'")
			execajaxa("delete from transkaskeluar where nid='"+mmmerkid+"'")
...
?>
	...
	<tr><td>Dari Rekening</td><td> : <?php combobox("select rekid misi,concat(rekid,'-',reknama) mtampil from setrek where nrcid in ('100','101') order by rekid","mrekid"); ?> </td></tr>
	</table>
