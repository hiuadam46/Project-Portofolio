...
<title>Daftar Faktur dan Tagihan</title>
...
$Pcs2(document).ready(function()
{
	...
	{
		mser=$Pcs2("#mnid").val();
		mggg=taptabel("transkasir1","date_format(tgl,'%d-%m-%Y') tgl,mobil","tagihanid='"+mser+"' limit 0,1")
...
function baru(mgdn)
{
	...
	datas=taptabel("transkasir1","tagihanid","tagihanid<>'' group by tagihanid order by tagihanid desc limit 0,1")
	...
	execajaxa("update transkasir1 set tagihanid='' where tagihanid='"+mmmerkid+"'")
...
<?php
require("menu.php");
execute("update transkasir1 set report=0");
?>
<form id="fform" name="fform" style="color: white; font-family: Arial; font-weight: bold">
	<table width="100%">
		<tr>
			<!--<td>Sales</td>-->
			<td>
			<!--:<?php combobox("select '' misi,'' mtampil union select karid misi,concat(karid,' - ',karnama) mtampil from setkaryawan order by misi","msalesid") ?>  -->
			Rute :<?php combobox("select '' misi,'' mtampil union select rute misi,rute mtampil from setlgn order by misi","msupid") ?></td>
		</tr>
		<tr>
			<td>Cari&nbsp;&nbsp;:
			<input id="mcari" onkeyup="refreshgrid2(this.value)" size="30" type="text"> 
			( No. Faktur / No. Outlet / Nama Outlet / Alamat )</td>
		</tr>
	</table>
</form>
<hr style="color: white">
<label style="color: white; font-family: Arial; font-weight: bold"><i>Semua Nota 
:</i></label> <span id="spantabel2"></span>
<label style="color: white; font-family: Arial; font-weight: bold"><i>Nota Dipilih 
:</i></label> <span id="spantabel3"></span>
<input id="mnota" size="50" type="hidden"><hr><center>
<input id="mprint1" onclick="rekapjualoutlet()" type="button" value="Print Daftar">
</center>
<div id="kotakdialog2" title="Setup Pelanggan">
	<center><iframe id="framehrg" height="400" style="top: 30px" width="100%">
	</iframe></center></div>
