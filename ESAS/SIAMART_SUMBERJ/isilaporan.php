...
$sqlstr="
select a.*, a.stoid,a.stonama,a.supid,a.isi,a.isi2,a.satuan1,a.satuan2,a.satuan3,
...
if($mlokid != ''){
	$lokasi = substr($mlokid, 3);
	$mgudang = '(Gudang :'.$row[kgudang.$lokasi].' Zona :'.$row[zona.$lokasi].' Rak :'.$row[rak.$lokasi].' Shelf :'.$row[shelf.$lokasi].' Cell :'.$row[cell.$lokasi].')';
}else{
	$mgudang = '';
}
...
<td bgcolor=#FFFFFF align=left><font size=2>$mreknama $mgudang</td>
...
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_sumberj_data' and TABLE_NAME='$mtabelnya'
";
...
{
$msql="SELECT COLUMN_NAME kolom,DATA_TYPE tipe,NUMERIC_PRECISION panjang1,CHARACTER_MAXIMUM_LENGTH panjang2,COLUMN_COMMENT koment
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_sumberj_data' and TABLE_NAME='$mtabelnya' and COLUMN_NAME in ($mkol)
";
}
...
if ($mLapo=='mlapjual2')
{
	...
	where tgl between str_to_date("'.$mtg1.'", "%d-%c-%Y") and str_to_date("'.$mtg2.'", "%d-%c-%Y") and total<>0 and a.nid like "!!'.$mnotaid.'!!" and a.lgnid like "!!'.$mlgnid.'!!"
	...
	<table width=100% cellspacing=0 cellpadding=0 bgcolor=white border=1 style="border-collapse:collapse; font-size:85%"> 
		<tr bgcolor="lightgrey">
			...
			<th>Tunai</th>
			<th>Piutang</th>
			<th style="display:none">Jt. Tempo</th>
		</tr>
	';
	...
	while ($mrr=mysql_fetch_object($maa))
	{
		...
		$mss=executerow("select a.*,b.stonama,b.satuan1,b.satuan2 from transkasir2 a left join setstok b on a.stoid=b.stoid where a.nid='$mrr->nid' and a.stoid like '$mstoid%' order by num");
		$mnomor=1;
		$mtotqty=0;
		...
		/* echo '
		<tr valign=top>
			<td colspan=3 align=right><b>TOTAL&nbsp<br>&nbsp;</td>
			...
			<td align=right style="display:none"><b>'.date( "d-m-Y", strtotime( $mrr->tgljt )).'</td>
		</tr>
		'; */
		echo '
		<tr valign=top>
			...
			<td align=right><b>'.number_format($mrr->total, 0, ".", ",").'</td>
			<td align=right><b>'.number_format($mrr->total, 0, ".", ",").'</td>
			<td align=right><b>'.number_format($tunai, 0, ".", ",").'</td>
			<td align=right><b>'.number_format($mrr->total-$tunai,0, ".", ",").'</td>
			<td align=right style="display:none"><b>'.date( "d-m-Y", strtotime( $mrr->tgljt )).'</td>
		</tr>
		';
		...
		$tottunai=$tottunai+$tunai;
		/* $tothutang=$tothutang+$mrr->jumlah; */
		$tothutang=$tothutang+$totot-$tottunai;
...
if ($mLapo=='mlapjualnota')
{
	...
	$sqlstr='
	select a.*, date_format(a.tgl, "%d-%m-%Y") tglc, date_format(a.tgl, "%d-%m-%Y") tgljtc, b.lgnid, b.bayar, b.jumlah, c.lgnnama, sum(a.jmlhrg) as jmlhrg, e.retur, f.lunas
	...
	left join (select nid2, sum(kredit) retur from bkbesar where rekid="10210" and trans="TRANSRETURJUAL" group by nid2) e on b.nid=e.nid2
	left join (select nid2,sum(kredit) lunas from bkbesar where rekid="10210" and trans="TRANSLUNASPIUTANG" group by nid2) f on b.nid=f.nid2
	where a.tgl between str_to_date("'.$mtg1.'", "%d-%c-%Y") and str_to_date("'.$mtg2.'", "%d-%c-%Y") and b.lgnid like "!!'.$mlgnid.'!!" and '.$ttk.' and d.grpid like "'.$mppn.'"
	group by nid
	';
	...
	{
		echo("
		TB. SUMBER JAYA MAKMUR 2 (PPN)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NPWP : 06.628.748.3-645.000
		<br>
		Laporan Penjualan Per Nota
		<br>Tgl. $mtg1 s.d. $mtg2 $mtgt
		<br><br>
	...
	{
		echo("
		TB. SUMBER JAYA MAKMUR 2 (NonPPN)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NPWP : 06.628.748.3-645.000
		<br>
		Laporan Penjualan Per Nota
		<br>Tgl. $mtg1 s.d. $mtg2 $mtgt
		<br><br>
...
$sql_penyesuaian = "
select a.*, b.* 
from transsesuai a left join setstok b on a.stoid=b.stoid
where a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') and a.lokid1 like '$mlokid%'
";
...
$rowawal = execute($sql_penyesuaian);
...
order by tgl,ts
";
$result2 = mysql_query ($sql_penyesuaian) or die ("Kesalahan pada perintah SQL JUAL1! ".mysql_error());
...
<td bgcolor=#CCCCCC align=center rowspan=2><b>No.</b></td>
<td bgcolor=#CCCCCC align=center rowspan=2><b>Tgl.</b></td>
<td bgcolor=#CCCCCC align=center rowspan=2><b>ID</b></td>
<td bgcolor=#CCCCCC align=center rowspan=2><b>Kode Stok</b></td>
<td bgcolor=#CCCCCC align=center rowspan=2><b>Nama Stok</b></td>
<td bgcolor=#CCCCCC align=center colspan=2><b>Jumlah Stok</b></td>
<td bgcolor=#CCCCCC align=center colspan=2><b>Penyesuaian Stok</b></td>
<td bgcolor=#CCCCCC align=center colspan=2><b>Selisih</b></td>
<td bgcolor=#CCCCCC align=center rowspan=2><b>Rp.</b></td>
...
while ($row = mysql_fetch_assoc ($result2))
{
$selisih_qty = $row[qtya]-$row[qty];
$selisih_unit = $row[unita]-$row[unit];
$harga_awal = ($row[hrgbeli]*$row[qty])+(($row[hrgbeli]/$row[isi])*$row[unit]);
$harga_akhir = ($row[hrgbeli]*$row[qtya])+(($row[hrgbeli]/$row[isi])*$row[unita]);
$selisih_harga = $harga_awal-$harga_akhir;
echo("
<tr>
<td bgcolor=#FFFFFF align=right >".$mnom.".</td>
<td bgcolor=#FFFFFF align=left > ".$row[tgl]."</td>
<td bgcolor=#FFFFFF align=left >".$row[nid]."</a></td>
<td bgcolor=#FFFFFF align=left >".$row[stoid]."</td>
<td bgcolor=#FFFFFF align=left >".$row[stonama]."</td>
<td bgcolor=#FFFFFF align=right >".$row[qty]."</td>
<td bgcolor=#FFFFFF align=right >".$row[unit]."</td>
<td bgcolor=#FFFFFF align=right >".$row[qtya]."</td>
<td bgcolor=#FFFFFF align=right >".$row[unita]."</td>
<td bgcolor=#FFFFFF align=right >".$selisih_qty."</td>
<td bgcolor=#FFFFFF align=right >".$selisih_unit."</td>
<td bgcolor=#FFFFFF align=right >".number_format($selisih_harga,0,',','.')."</td>
</tr>
");
$total_selisih = $total_selisih + $selisih_harga;
...
echo("
<tr>
<td bgcolor=#FFFFFF align=right colspan=11><b>TOTAL&nbsp;</td>
<td bgcolor=#FFFFFF align=right ><b>".number_format($total_selisih,0,',','.')."</b></td>
</tr>
</table>
</font>
");
