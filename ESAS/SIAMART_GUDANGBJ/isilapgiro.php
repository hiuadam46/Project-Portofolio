...
and rekid='$mrekid' and debet<>0 order by tgl2
";
}
else
{
...
where a.tgl2 between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y')
and rekid='$mrekid' and kredit<>0 order by tgl2
";
}
...
$toba=execute("select sum(debet-kredit) saldo,rekid from bkbesar where rekid in (select rekid from setrek where rekid like '10110%' )  and tgl <= str_to_date('$mtg2','%d-%c-%Y') order by rekid limit 0,1");
