...
<html><head>
	...
	function buatlaporan(ngeprint, excel)
	{
		...
		if(!excel){
			if (!ngeprint)
			{
			$Pcs2("#framelap").attr("src","isilaporan.php?mLapo=mlapjualkiriman&mtg1="+mtgl1+"&mtg2="+mtgl2+"&mlgnid="+asd+"&mkasir="+$Pcs2("#mkasir").val())
			}
			else
			{
			window.open("isilaporan.php?mLapo=mlapjualkiriman&mtg1="+mtgl1+"&mtg2="+mtgl2+"&isprint=YY"+"&mlgnid="+asd+"&mkasir="+$Pcs2("#mkasir").val())
			}
		}
		else{
			window.open("isilaporan.php?mLapo=mlapjualkiriman&excel=true&mtg1="+mtgl1+"&mtg2="+mtgl2+"&isprint=YY"+"&mlgnid="+asd+"&mkasir="+$Pcs2("#mkasir").val())
		}
...
?>	
	<form name='fform' id='fform'>
		...
		<div style="position:relative; width:1000px;text-align:left;background-color:sBlue">
			<table width='100%'>
				...
				<tr>
					<td>
						Kasir : <input type=text id=mkasir size=8><input type='button' id=mlkasir value='F5'><input type=text id=mkasirnama readonly>
	...
	<iframe id='framelap' width='100%' height='450px' src="isilaporan.php">
