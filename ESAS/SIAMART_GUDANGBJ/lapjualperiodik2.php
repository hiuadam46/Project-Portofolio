...
$Pcs2(document).ready(function()
{
	...
	$Pcs2("#jtgl1").datepicker({dateFormat: 'dd-mm-yy'});
	$Pcs2("#jtgl2").datepicker({dateFormat: 'dd-mm-yy'});
...
function buatlaporan(ngeprint)
{
	...
	jtgl1=$Pcs2("#jtgl1").val()
	jtgl2=$Pcs2("#jtgl2").val()
	
	if (!ngeprint)
	{
		$Pcs2("#framelap").attr("src","isilaporan.php?mLapo=mlapjualnota&mtg1="+mtgl1+"&jtg1="+jtgl1+"&mtg2="+mtgl2+"&mlgnid="+asd+"&mkarid="+sal+"&mstoid="+$Pcs2("#mstoid").val())
	}
	else
	{
		window.open("isilaporan.php?mLapo=mlapjualnota&mtg1="+mtgl1+"&mtg2="+mtgl2+"&jtg1="+jtgl1+"&jtg2="+jtgl2+"&isprint=YY"+"&mlgnid="+asd+"&mkarid="+sal+"&mstoid="+$Pcs2("#mstoid").val())
	}
}
...
<form id="fform" name="fform">
	<div style="position: relative; width: 1000px; text-align: left; color: white; font-family: Arial">
		<table width="100%">
			<tr>
				...
				<img alt="" src="images/icon_print.png"> </a></td>
			</tr>
			<tr>
				<td>Pelanggan : <input id="mlgnid" size="8" type="text"><input id="mllgn" type="button" value="F5"><input id="mlgnnama" readonly type="text"> 
				Stok :
				...
				<hr>Tgl Jatuh Tempo :
				<input id="jtgl1" readonly size="10" type="text">
				<input id="jtgl2" readonly size="10" type="hidden"></td>
			</tr>
		</table>
	</div>
</form>
