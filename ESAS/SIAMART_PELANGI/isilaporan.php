...
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_pelangi_data' and TABLE_NAME='$mtabelnya'
";
...
{
$msql="SELECT COLUMN_NAME kolom,DATA_TYPE tipe,NUMERIC_PRECISION panjang1,CHARACTER_MAXIMUM_LENGTH panjang2,COLUMN_COMMENT koment
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_pelangi_data' and TABLE_NAME='$mtabelnya' and COLUMN_NAME in ($mkol)
";
}
...
if ($mLapo=='mlapjual3_baru')
{
	...
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and total<>0 and a.nid like '!!$mnotaid!!' order by a.tgl, a.jam, a.nid
	";
...
if ($mLapo=='mlapjualnota')
{
	...
	{
		echo("
		PELANGI MART (PPN)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NPWP : 06.628.748.3-645.000
		<br>
		Laporan Penjualan Per Nota
		<br>Tgl. $mtg1 s.d. $mtg2 $mtgt
		<br><br>
	...
	{
		echo("
		PELANGI MART (NonPPN)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NPWP : 06.628.748.3-645.000
		<br>
		Laporan Penjualan Per Nota
		<br>Tgl. $mtg1 s.d. $mtg2 $mtgt
		<br><br>
...
if ($mLapo=='mlapjualkasir')
{
	...
	//";
	/////////////////////////14JAN2020
	$sqlstr="select a.bpid stoid,sum(a.qtyout-a.qtyin) qty,sum(a.kredit-a.debet) hpp,b.stonama,b.isi,b.satuan1,b.satuan2,sum(d.jmlhrg) jmlhrg from (select nid,stoid,max(trans) trans,max(rekid) rekid,max(tgl) tgl,max(bpid) bpid,sum(qtyout) qtyout,sum(qtyin) qtyin,sum(kredit) kredit,sum(debet) debet from bkbesar group by nid,bpid) a left join setstok b on a.bpid=b.stoid left join transkasir1 c on a.nid=c.nid left join transkasir2 d on a.nid=d.nid and a.bpid=d.stoid where a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') and b.grpid like '%$mgrpid%' and c.kasir like '%$mkasir%' and c.lgnid like '%$mlgnid%' and a.trans='TRANSKASIR' and a.rekid like '103%' and $ttk and b.supid like '$msupid%' group by a.bpid,if(a.qtyout>0,'A','B') order by $urut ";
	/////////////////////////14JAN2020
