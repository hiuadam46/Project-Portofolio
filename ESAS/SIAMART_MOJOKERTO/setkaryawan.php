...
<meta http-equiv="content-type" content="text/html; charset=ISO-8859-1">
  ...
  $Pcs2(document).ready(function(){
	...
	$Pcs2("#btnkeluar2").click(function() {
		$Pcs2("#kotakdialog3").dialog('close');
	})
	...
	for (var mii=1;mii<30;mii++)
	{
		...
		function btnprint(){
			idgaji = $Pcs2("#idgaji").val();
		namagaji = $Pcs2("#namagaji").val();
		...
		gajipokok = $Pcs2("#gajipokok").val();
		santunankhusus = $Pcs2("#santunankhusus").val();
		uanglembur = $Pcs2("#uanglembur").val();
		uangmakan = $Pcs2("#uangmakan").val();
		tambahanlain = $Pcs2("#tambahanlain").val();
		jumlahgaji = parseInt(gajipokok)+parseInt(santunankhusus)+parseInt(uanglembur)+parseInt(uangmakan)+parseInt(tambahanlain);
		pajakpenghasilan = $Pcs2("#pajakpenghasilan").val();
		iuranjamsostek = $Pcs2("#iuranjamsostek").val();
		potonganlain = $Pcs2("#potonganlain").val();
		jumlahpotongan = parseInt(pajakpenghasilan)+parseInt(iuranjamsostek)+parseInt(potonganlain);
		sisadibayar = parseInt(jumlahgaji)-parseInt(jumlahpotongan);
		// alert("bisa");
		// alert(tanggal);
		// return;
		sql1 = "insert into gajikaryawan (karid, nama, tanggal, gajipokok, santunankhusus, uanglembur, uangmakan, tambahanlain, jumlahgaji, pajakpenghasilan, iuranjamsostek, potonganlain, jumlahpotongan, sisadibayar) ";
		sql2 = " values('"+idgaji+"','"+namagaji+"','"+tanggal+"','"+gajipokok+"','"+santunankhusus+"','"+uanglembur+"','"+uangmakan+"','"+tambahanlain+"','"+jumlahgaji+"','"+pajakpenghasilan+"','"+iuranjamsostek+"','"+potonganlain+"','"+jumlahpotongan+"','"+sisadibayar+"')";
		execajaxas(sql1+sql2);
		window.open('printgaji.php?idgaji='+idgaji+'&namagaji='+namagaji+'&tanggal='+tanggal+'&gajipokok='+gajipokok+'&santunankhusus='+santunankhusus+'&uanglembur='+uanglembur+'&uangmakan='+uangmakan+'&tambahanlain='+tambahanlain+'&jumlahgaji='+jumlahgaji+'&pajakpenghasilan='+pajakpenghasilan+'&iuranjamsostek='+iuranjamsostek+'&potonganlain='+potonganlain+'&jumlahpotongan='+jumlahpotongan+'&sisadibayar='+sisadibayar, 'aa', ('alwaysraised=yes, scrollbars=no, resizable=no'));
		xxw.focus();
		}

		$Pcs2("#btnprint").click(function() {
		// alert('harusnya keluar sih');
		btnprint();
		document.getElementById('btnkeluar2').click();
		return;
		})
...
///awal functions
	...
	function openPayroll(id){
		$Pcs2("#kotakdialog3").dialog('open');
		// document.getElementById("kotakdialog3").title = "Item Added";
		console.log(document.getElementById("kotakdialog3").title);
		datas=taptabel("setkaryawan","karid","1=1 order by karid desc limit 1");
		data=datas.karid;
		$Pcs2("#gajipokok").select()
		
		$Pcs2.ajax({
		type:"POST",
		url:"phpexec.php",    
		data: "func=GETSALES&id="+id+"",
		dataType:'json',
		success: function(data){
			$Pcs2("#gajipokok").val(data.gaji);
			$Pcs2("#namagaji").val(data.karnama);
			$Pcs2("#idgaji").val(data.karid);
			console.log(data);
		}
		})
	}
...
?>
	...
	FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_mojokerto_data' and TABLE_NAME='setkaryawan' and COLUMN_COMMENT<>''
	...
	<!-- KOTAK DIALOG 3 -->
	<div id="kotakdialog3" title="Setup Karyawan/Sales">	
	<form name='fsetgaji' id='fsetgaji'>
	<font size=4>
	...
	<tbody>
		<input align='right' type="text" name="idgaji"  id="idgaji" style='width:100%' hidden>
		<input align='left' type="text" name="namagaji"  id="namagaji" style='width:100%' disabled readonly>
		<tr>
			<td>
				Gaji Pokok:<input align='right' type="number" name="gajipokok" id="gajipokok" style='width:98%' value=0>
			</td>
			<td>
				Pajak Penghasilan:<input align='right' type="number" name="pajakpenghasilan"  id="pajakpenghasilan" style='width:98%' value=0>
			</td>
		</tr>
		<tr>
			<td>
				Santunan Khusus:<input align='right' type="number" name="santunankhusus" id="santunankhusus" style='width:98%' value=0>
			</td>
			<td>
				Iuran JAMSOSTEK:<input align='right' type="number" name="iuranjamsostek"  id="iuranjamsostek" style='width:98%' value=0>
			</td>
		</tr>
		<tr>
			<td>
				Uang Lembur:<input align='right' type="number" name="uanglembur" id="uanglembur" style='width:98%' value=0>
			</td>
			<td>
				Potongan Lain-lain:<input align='right' type="number" name="potonganlain"  id="potonganlain" style='width:98%' value=0>
			</td>
		</tr>
		<tr>
			<td>
				Uang Makan:<input align='right' type="number" name="uangmakan" id="uangmakan" style='width:98%' value=0>
			</td>
			<td>
			</td>
		</tr>
		<tr>
			<td>
				Tambahan Lain-lain:<input align='right' type="number" name="tambahanlain" id="tambahanlain" style='width:98%' value=0>
			</td>
			<td>
			</td>
		</tr>
	...
	<input type=button id='btnkeluar2' value=Tutup>
	</form>
	</div>
