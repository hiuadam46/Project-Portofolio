...
<html><head>
	...
	if (aaa==undefined)
	{break;}
	bbb=$Pcs2("#m_"+mii).attr('title')
	//bbb=bbb.replace(" ","_")
	//bbb=bbb.replace(" ","_")
	//bbb=bbb.replace(" ","_")
	
	//alert("update setinterface set isi='"+aaa+"' where ket='"+bbb+"'")

	execajaxa("update setinterface set isi='"+aaa+"' where ket='"+bbb+"'")
...
?>	
	...
	echo "<td  >$row[ket]</td><td> ";
	echo ": <input  type=text id='m_$mnom' title='$row[ket]' value='$row[isi]'>";
	echo "</td>";
	echo "</tr>";
	$mnom++;
