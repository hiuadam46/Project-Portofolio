	...
	return;	
		...
		if (! (this.value=='A' || this.value=='B' || this.value=='C' ))
		...
		else
			{
				...
				if (this.value=='A')
				{
					$Pcs2("#mKeterangan").val("-")
				}
				else
				{
					if (this.value=='B')
					{
						$Pcs2("#mKeterangan").val("GROSIR")
					}
					else
					{
						if (this.value=='C')
						{
							$Pcs2("#mKeterangan").val("RETAIL")
						}
					}
				}
...
?>
	...
	FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_majapahit_data' and TABLE_NAME='setlgn' 
	and COLUMN_COMMENT<>''
