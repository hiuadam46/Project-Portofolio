...
<head>
    ...
    <script type="text/javascript">
	  ...
	  var dialogisopen=false;
		...
		//$Pcs2("#mlokid").attr('disabled',true)
		...
		if (mnn=='2' && mmv==116)
		{		
		query1="select left(a.stoid,12) stoid,left(a.barcode,20) Barc,left(a.stonama,30) Nama,left(format(a.hrgbeli,0),15) rightHarga,'"+$Pcs2("#mlokid").val()+"' QSALDO from setstok a "
		query2=" where true "
   ...
   $Pcs2("#kotakdialog2").dialog({
	...
	$Pcs2("#mnid").blur(function() {
			...
			if (mnid!=undefined)	
			{	
				//alert("masuuk kondisi pak ekoooo")
				$Pcs2("#mtgl").val(tb1.tgly)
				$Pcs2("#mlokid").val(tb1.lokid1)
				$Pcs2("#mlokid").blur()
...
////////////////////////////CEK APAKAH ADA DOUBLE ENTRI MATERIAL/////////////////////////////////////////
		mnnm=toval(mid.replace('2_',''))
		for (ggt=1;ggt<101;ggt++)
		{
			msst=$Pcs2("#2_"+ggt).val()
			mnama=$Pcs2("#3_"+ggt).val()
...
?>
	...
	<td>Lokasi Stok</td>
	<!--<td> : <input type=text  size=5 id='mlokid' value='01'> -->
	<td> : <input type=text  size=5 id='mlokid'>
	...
	<tr>
	<td>No.</td><td> : <input type=text id=mnid size=10> 
	</input><input type=button id=lookmnid value='F5'> </td>
	...
	</input> <input type=button id='btnbaru' value='Baru'>
	</input> <input type=button id='btnhapus' value='Hapus'>
	</input> <input type=button id='btnkeluar' value='Keluar'>
	</input>
	</form>
