...
$Pcs2(document).ready(function()
{
	...
	$Pcs2(document).keypress(function()
	{
		...
		if (mmv==114)
		{
			...
			ambilstok(msebel, 'f3')
			if(document.getElementById("13_"+mss).value == ''){
				$Pcs2("#stoid_"+mss).val('')
				$Pcs2("#13_"+mss).val('')
				$Pcs2("#14_"+mss).val('')
				$Pcs2("#15_"+mss).val('')
	...
	$Pcs2("#fform").keydown(function()
	{
		...
		if (mmv==116 && (mmid.substr(0,2)=='12' || mmid.substr(0,2)=='32') )
		{
			...
			if (hakedit==1)
			{
				query1="select left(a.stoid, 10) K, left(a.barcode, 15) Barc, left(a.stonama, 48) Nama, left(format(a.hrgjual"+mha+", 0), 12) rightHarga, 'L001' QSALDO, stoid from setstok a "
				query2=" where true "
			}
			else
			{
				query1="select left(a.stoid, 10) K, left(a.barcode, 10) Barc, left(a.stonama, 46) Nama, left(format(a.hrgjual"+mha+", 0), 12) rightHarga from setstok a "
				query2=" where true "
			}
			query1="select left(a.stoid, 10) K, left(a.barcode, 10) Barc, left(a.stonama, 46) Nama, left(format(a.hrgjual"+mha+", 0), 12) rightHarga, 'L001' QSALDO, stoid from setstok a "
			query2=" where true "
	...
	$Pcs2("#btnsimpan").click(function()
	{
		...
		mq1="insert into transkasir1(nid, tgl, jam, kasir, total, diskon, jumlah, edc, bayar, kembali, status, kartu, referensi1, referensi2, lgnid, golongan) "
		mq2=" values('"+mnid+"', "+mtgl+", '"+mjam+"', '"+mkasir+"', '"+mtotal+"', '"+mdiskon+"', '"+mjumlah+"', '"+medc+"', '"+mbayar+"', '"+mkembali+"', 2, '"+mkartu+"', '"+mref1+"', '"+mref2+"', '"+mlgnid+"', '"+mgol+"')"
		execajaxas(mq1+mq2)
	...
	$Pcs2("#btnsimpandiskon").click(function()
	{
		...
		if (vll=='')
		{
			vidn=toval(lastobj.replace('12_',''))-1
			vid=vidn.toString()
			mid="17_"+vid
		}
		else
		{
			vidn=toval(lastobj.replace('12_',''));
			vid=vidn.toString();
...
function arahsatuan(theid)
{
	...
	$Pcs2("#btnsemiecer").css("color","black")
	msatuan=theid
	mgolongan=$Pcs2("#mgolongan").val()
	...
	{
		arahharga(4)
		$Pcs2("#btnsemiecer").css("color","red")
	}
}
...
function baru(mgdn)
{
	...
	$Pcs2("#lblwalk").html("UD. ANGEL Siap Melayani Anda")
...
function ambilstok(theid, key)
{
	...
	golonganuser = $Pcs2('#mgolongan').val();
	...
	mfield="stoid, barcode, barcode2, barcode3, stonama, FORMAT(hrgbeli/if("+msatuan+"=1, 1, isi), 0) hrgbeli, satuan"+msatuan+" satuan, FORMAT(hrgjual"+mha+"/if("+msatuan+"=1, 1, isi), 0) hrgjual, 0 diskon, if("+msatuan+"=1, isi, 1) isi, b.poin_rp"
	if(key == 'f7'){
		mfield="stoid, barcode, barcode2, barcode3, stonama, FORMAT(hrgbeli/if("+msatuan+"=1, 1, isi2), 0) hrgbeli, satuanb1 satuan, FORMAT((hrgjual"+mha+"), 0) hrgjual, 0 diskon, isi2 isi, b.poin_rp"
		
	}
	if(key == 'f4'){
		mfield="stoid, barcode, barcode2, barcode3, stonama, FORMAT(hrgbeli/if("+msatuan+"=1, 1, isi2), 0) hrgbeli, satuanc1 satuan, FORMAT((hrgjual"+mha+"), 0) hrgjual, 0 diskon, isi3 isi, b.poin_rp"
		
	}
	if(key == undefined){
		mfield="stoid, barcode, barcode2, barcode3, stonama, FORMAT(hrgbeli/if("+msatuan+"=1, 1, isi2), 0) hrgbeli, satuanc1 satuan, FORMAT((hrgjual"+gol+"3), 0) hrgjual, 0 diskon, isi3 isi, b.poin_rp"
		
	}
	mtabel="setstok a left join setgrp b on a.grpid=b.grpid "

	mkondisi="stoid='"+misi+"' or barcode='"+misi+"' or barcode2='"+misi+"' or barcode3='"+misi+"'"
	...
	$Pcs2.ajax(
	{
		...
		{
			mstok=data
			golongannya = mgolongan.toLowerCase()
			if (mstok.stonama!=undefined )
			{
				...
				if (misi==mstok.barcode2)
				{
					...
					arahsatuan(1);
					arahharga(2);
					mfield="stoid, barcode, barcode2, barcode3, stonama, FORMAT(hrgbeli/if("+msatuan+"=1, 1, isi2), 0) hrgbeli, satuanb1 satuan, FORMAT((hrgjual"+golongannya+"2), 0) hrgjual, 0 diskon, if("+msatuan+"=1, isi2, isi2) isi, b.poin_rp"
					mstok=taptabel(mtabel, mfield, mkondisi)
				}

				// console.log(mstok);
				if (misi==mstok.barcode3)
				{
					// alert(golongannya)
					if (golongannya == 'a'){
						golongannya = ''
					...
					// alert("ini barcod22e 2")
					arahsatuan(1);
					arahharga(2);
					mfield="stoid, barcode, barcode2, barcode3,  stonama, FORMAT(hrgbeli/isi3, 0) hrgbeli, satuanc1 satuan, FORMAT(hrgjual"+golongannya+"3, 0) hrgjual, 0 diskon, if("+msatuan+"=1, isi3, isi3) isi, b.poin_rp"
					mstok=taptabel(mtabel, mfield, mkondisi)
				}
				...
				if (misi==mstok.barcode)
				{
					...
					arahsatuan(2);
					arahharga(3);
					mfield="stoid, barcode, barcode2, barcode3, stonama, FORMAT(hrgbeli/if("+msatuan+"=1, 1, isi), 0) hrgbeli, satuan1 satuan, FORMAT(hrgjual"+golongannya+", 0) hrgjual, 0 diskon, if("+msatuan+"=2, isi, 1) isi, b.poin_rp"
					mstok=taptabel(mtabel, mfield, mkondisi)
				}
				...
				mtabel1="setstok "
				mfield1="* "
				mkondisi1="stoid='"+misi+"'"
				
				satuanqty=taptabel(mtabel1, mfield1, mkondisi1)
...
function rumus1(mttt)
{
	...
	for (gg=1;gg<100;gg++)
	{
		...
		if ($Pcs2("#stoid_"+gg).val()!='')
		{
			...
			//if (mhrg<hrgb)
			if (mhrg<hrgb && vidn==gg)
			{
				var x=window.confirm("Diskon "+mstonama+" Terlalu Besar !"+"\n\n"+"Batal?"+"\n\n"+"OK untuk batal diskon, Cancel untuk lanjut diskon.."+"\n");
				if(x)
				{
					$Pcs2("#17_"+gg).val('0%')
					mhrg=toval($Pcs2("#16_"+gg).val());
...
<form id="fform" name="fform">
	<table border="0" style="border-collapse: ; color: white; font-family: Arial; font-weight: bold" width="100%">
		...
		<tr>
			...
			<label id="lblwalk" style="font-size: 18pt">UD. ANGEL 
		...
		<tr>
			...
			<table id="mtab1" border="0" style="font-size: 12pt; background-color: #A52A2A" width="100%">
				...
				<tr>
					...
					<?php
					combobox("select reknama misi,concat(reknama) mtampil from setrek where rekid like '101%' order by rekid","mkartu");
					?>
...
<b>
	<input type='button' value='F2-Print Ulang' id='btnprintulang'>	: <input type=text id='mnidold' size=8>	
	</b>
<input id="btnpartai" type="button" value="F3-H1">
<input id="btnsemiecer" type="button" value="F7-H2">
<input id="btnecer" type="button" value="F4-H3">
