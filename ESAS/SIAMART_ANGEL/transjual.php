...
$Pcs2(document).ready(function()
{
	...
	$Pcs2("#fform").keydown(function()
	{
		...
		if (mmid.substr(0,2)=='12' && (mmv==116 || (mmv==13 && mmf2.value=='') ))
		{
			...
			query1="select left(a.stoid,12) stoid, left(a.stonama,45) Nama, left(format(a.hrgjual"+mh+",0),12) rightHarga, '"+mlokid+"' QSALDO from setstok a "
			query2=" where true "
	...
	{
		mcomm="Select rpad(lgnid,12,' ') Kode,left(lgnnama,50) Nama,rpad(golongan,12,' ') rightGol,alamat from setlgn where true"
		mordd="Kode"
	...
	$Pcs2("#btnsimpan").click(function()
	{
		...
		mq1="insert into transjual1(nid,tgl,tgljt,ket,lgnid,total,tunai,jumlah,ongkos,sales,status, golongan) "
		mq2=" values('"+mnid+"',"+mtgl+","+mjt+",'"+mket+"','"+mlgnid+"','"+mtotal+"','"+mtunai+"','"+mhutang+"','"+mongkos+"','"+mkarid+"',2,'"+mgol+"')"
		execajaxas(mq1+mq2)
...
function cekqtyx(thiee) 
{
	...
	misi2=toval($Pcs2("#misi2_"+m23).val())
	mtotqty=(((mqty)*misi)+mextra*misi2)+munit+mextraunit
...
function baru(mgdn)
{
	...
	$Pcs2("#mlgnid").focus()
	mlkk=taptabel("setlok","lokid,loknama","lokid='L002' order by lokid limit 0,1")
...
function ambilstok(theid, key)
{
	...
	mstok=taptabel("setstok a ","a.stoid,a.stonama,a.satuan1,a.satuanb1,a.satuanc1,format(a.hrgjual"+mh+",0) hrgjual, format(a.hrgjual"+mh+"2,0) hrgjual2,a.isi, a.isi2","a.stoid='"+misi+"'")
	...
	if (mstok.stonama!=undefined)
	{
		...
		$Pcs2("#"+rowsat2).val(mstok.satuanc1)
		$Pcs2("#"+rowsat3).val(mstok.satuanb1)
...
function rumus1(key)
{
	...
	for (gg=1;gg<100;gg++)
	{
		if ($Pcs2("#12_"+gg).val()!='')
		{
			...
			misi=toval($Pcs2("#misi_"+gg).val())
			misi2=toval($Pcs2("#misi2_"+gg).val())
			mtotqty=(mqty*misi)+(mextra*misi2)+munit+mextraunit
			...
			mjmlhrg=(((mqty*misi)+(mextra*misi2))+munit)*mhrg
...
function cekdiskon(tag)
{
	...
	mstok=taptabel("setstok", "hrgbeli, isi", "stoid='"+mstoid+"'");
	//alert(mqty);
	if(mqty>0)
	{
		mhrgbeli=toval(mstok.hrgbeli);
	}
	else
	{
		mhrgbeli=toval(mstok.hrgbeli)/toval(mstok.isi);
	}
