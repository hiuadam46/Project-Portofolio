...
function buatlaporan(ngeprint, excel)
{
	...
	if(!excel)
	{
		if (!ngeprint)
		{
			$Pcs2("#framelap").attr("src","isilaporan.php?mLapo=mlapjualnota&mtg1="+
			mtgl1+"&mtg2="+mtgl2+"&mlgnid="+asd+"&mtk="+sal+"&mppn="+
			mppn+"&mstoid="+$Pcs2("#mstoid").val()+"&mkasir="+mkasir);
		}
		else
		{
			window.open("isilaporan.php?mLapo=mlapjualnota&mtg1="+mtgl1+"&mtg2="+
			mtgl2+"&isprint=YY"+"&mlgnid="+asd+"&mtk="+sal+"&mppn="+mppn+"&mstoid="+
			$Pcs2("#mstoid").val()+"&mkasir="+mkasir);
		}
	}
	else
	{
		window.open("isilaporan.php?mLapo=mlapjualnota&excel=true&mtg1="+mtgl1+"&mtg2="+
		mtgl2+"&isprint=YY"+"&mlgnid="+asd+"&mtk="+sal+"&mppn="+mppn+"&mstoid="+
		$Pcs2("#mstoid").val()+"&mkasir="+mkasir);
	}
}
...
<form id="fform" name="fform">
	<div>
		<table style="position: relative; width: 100%; text-align: left; color: white; font-family: Arial;border:0px;width:100%">
			<tr>
				<td> Tgl.</td>
				<td>:<input id="mtgl1" readonly size="10" type="text"> 
			...
			<tr>
				<td>Pelanggan </td> 
				<td colspan=5>:<input id="mlgnid" size="8" type="text">
					<input id="mllgn" type="button" value="F5">
					<input id="mlgnnama" readonly type="text"> 
				Kasir 	
				:<input id="mkasir" size="8" type="text">
					<input id="mlkasir" type="button" value="F5">
					<input id="mkasirnama" readonly type="text"> 
				<!-- Stok : -->
				<!-- <input type=text id=mstoid size=8 onblur=buatlaporan()> -->
				<!-- <input type='button' id=mlookstok value='F5' onclick=bukastok() > -->
				<!-- <input type=text id=mstonama readonly> -->
				Jenis : <select id="mtk">
...
<div id="kotakdialog2" title="Lookup pelanggan">
	<iframe id="framehrg" height="450" width="100%" style=" box-align:center"></iframe>
</div>
