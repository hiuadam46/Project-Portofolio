...
<html><head>
    ...
    <script type="text/javascript">
	  ...
	  var dialogisopen=false;
		...
		var select = document.getElementById("mlokid1");
		
		var option = document.createElement('option');
		option.text = 'Lokasi Awal'
		option.value = 'default';
		select.add(option, 0);

		var select2 = document.getElementById("mlokid2");

		var option2 = document.createElement('option');
		option2.text = 'Lokasi Tujuan'
		option2.value = 'default';
		select2.add(option2, 0);
		$Pcs2("#mlokid1").val('default');
		$Pcs2("#mlokid2").val('default');
	...
	$Pcs2("#msupid").blur(function() {
		...
		$Pcs2("#mlokid1").change(function(){
			buatlaporan()
		})

		$Pcs2("#mlokid2").change(function(){
			buatlaporan()
		})
	...
	function buatlaporan(ngeprint)
	{
			...
			asal=$Pcs2("#mlokid1").val()
			tujuan=$Pcs2("#mlokid2").val()

			if (!ngeprint)
			{
			$Pcs2("#framelap").attr("src","isilaporan.php?mLapo=mlapmutasi&awal="+asal+"&tujuan="+tujuan+"&mtg1="+mtgl1+"&mtg2="+mtgl2+"&msupid="+asd+"&mstoid="+$Pcs2("#mstoid").val())
			}
			else
			{
			window.open("isilaporan.php?mLapo=mlapmutasi&awal="+asal+"&tujuan="+tujuan+"&mtg1="+mtgl1+"&mtg2="+mtgl2+"&isprint=YY"+"&msupid="+asd+"&mstoid="+$Pcs2("#mstoid").val())
			}
...
?>	
	...
	<td>:
	<?php
	combobox("select loknama misi,concat(loknama,' - ',lokid) mtampil from setlok order by urutan","mlokid1");
	?>
	</td>
	<td>Lokasi Tujuan</td>
	<td>:
	<?php
	combobox("select loknama misi,concat(loknama,' - ',lokid) mtampil from setlok order by lokid","mlokid2");
	?>
	<td align=right><a href="" id='goprint' > <img src="images/icon_print.png">	
	</tr>
