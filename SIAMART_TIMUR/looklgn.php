...
<meta http-equiv="content-type" content="text/html; charset=ISO-8859-1">
	...
	$Pcs2(document).keydown(function(){
		...
		//alert(mmf)
			...
			if (mobj=='setstok1')
			{
			opener.fsetstok.mlgnid.value=isi;
			self.close();
			opener.focus();
			opener.fsetstok.mlgnid.focus();
			opener.fsetstok.mlgnid.blur();
			opener.fsetstok.mlgnid.select();
			return;
			}

			if (mobj=='looklgntagihan')
			{
			parent.refreshgrid2(isi);
			...
			if (mobj=='looklgnjual')
			{
			parent.fform.mlgnid.value=isi;
			parent.fform.mlgnnama.value=isi2;
			parent.settinglgn()
			parent.fform.mmobilid.focus();
			parent.$Pcs2("#kotakdialog2").dialog("close");			
			return;
			}

			if (mobj=='looklgnpiutang')
			{
			parent.fform.mlgnid.value=isi;
			parent.fform.mlgnnama.value=isi2;
			parent.$Pcs2("#mlgnid").blur();
			parent.fform.mket.focus();
			parent.$Pcs2("#kotakdialog2").dialog("close");			
			return;
			}
			
			if (mobj=='looklgnreturjual')
			{
 			parent.fform.mlgnid.value=isi;
			parent.fform.mlgnnama.value=isi2;
			parent.fform.mlgnid.blur();
		...
		tableGrid1.request="msql=select * from setlgn where concat(lgnid,lgnnama,alamat) like '!!"+mCari+"!!'";
...
<?php
	echo("request:".'"msql=select * from setlgn where concat(lgnid,lgnnama) like'."'!!".$mCari."!!'".'"');
?>	
    };
