...
<meta http-equiv="content-type" content="text/html; charset=ISO-8859-1">
  ...
  $Pcs2(document).ready(function(){
	...
	$Pcs2("#fform").keydown(function() {	
		...
		query1="select left(a.stoid,12) stoid,left(a.stonama,48) Nama,left(format(a.hrgbeli,0),15) rightHarga,'L001' QSALDO from setstok a "
		query2=" where true "
	...
	$Pcs2("#mbeliid").blur(function() {
		...
		if (mndd!='')
		{
			...
			datalok=taptabel("transbeli1","nid,lokid","nid='"+mndd+"'")
	...
	{
		xxw=window.open('printreturbeli.php?mnid='+mnid, 'aa', ('alwaysraised=yes, scrollbars=no, resizable=no, height=550, left=200, top=10'));
		xxw.focus();
	})
	...
	$Pcs2("#lookbeli").click(function() {
		...
		mcomm="Select nid Nota,date_format(tgl,'%d-%m-%Y') Tgl,FORMAT(kredit,0) rightJumlah  from bkbesar where bpid='"+msup+"' and rekid='20010' and kredit>0"
		mcomm="Select a.nid Nota,date_format(a.tgl,'%d-%m-%Y') Tgl,FORMAT(a.kredit-ifnull(b.debet,0),0) rightJumlah  from bkbesar a left join (select nid2,sum(debet) debet from bkbesar where bpid='"+msup+"' and rekid='20010' and debet>0 group by nid2 )  b on a.nid=b.nid2 where bpid='"+msup+"' and rekid='20010' and kredit>0 and a.kredit-ifnull(b.debet,0)<>0"
		mordd="Nota"
		mffff=" nid "
	...
	$Pcs2("#btnsimpan").click(function() {
		...
		success : function(data){
			...
			mq1="insert into transreturbeli1(nid,tgl,beliid,ket,supid,total,status,lokid) "
			mq2=" values('"+mnid+"',"+mtgl+",'"+mbeliid+"','"+mket+"','"+msupid+"','"+mtotal+"',2,'"+mlokid+"')"
			execajaxa(mq1+mq2)
			...
			mjm=savstofifo(mlokid,mstoid,mnid,mtglx,0,0,0,mtotqty,'TRANSRETURBELI',"Retur Pembelian")
...
?>
	...
	Tgl : <input type=text id=mbelitgl size=10 disabled> 
	&nbsp;&nbsp;&nbsp;Lokasi : <input type=text  size=5 id='mlokid'><input type=button id=looklok value='F5' ><input type=text id=mloknama size=15 disabled>
	</td> 
	</tr>
