...
<script type="text/javascript">
 ...
 $Pcs2("#kotakdialog2").dialog({
	...
	$Pcs2("#fform").keydown(function() {	
		...
		query1="select left(a.stoid,12) stoid,left(a.stonama,45) Nama,left(format(a.hrgjual"+mh+",0),15) rightHarga,'L001' QSALDO from setstok a "
		query2=" where true "
	...
	$Pcs2("#mjualid").blur(function() {
		...
		datalok=taptabel("transjual1","lokid","nid='"+mndd+"'")
	...
	{
		xxw=window.open('printreturjual.php?mnid='+mnid, 'aa', ('alwaysraised=yes, scrollbars=no, resizable=no, height=550, left=200, top=10'));
		xxw.focus();
	})
	...
	$Pcs2("#lookjual").click(function() {
		...
		mcomm = " select a.nid Nota,date_format(a.tgl,'%d-%m-%Y') Tgl, format(a.jumlah-ifnull(b.bayar,0)-ifnull(c.retur,0),0) rightJumlah from transjual1 a left join "
		mcomm = mcomm + " (select nid2,sum(kredit) as bayar from bkbesar where bpid='"+mlgn+"' and rekid='10210' and trans='TRANSLUNASPIUTANG' group by nid2) b "
		mcomm = mcomm + " on a.nid=b.nid2 left join "
		mcomm = mcomm + " (select nid2,sum(kredit) as retur from bkbesar where bpid='"+mlgn+"' and rekid='10210' and trans='TRANSRETURJUAL' "
		mcomm = mcomm + " group by nid2) c on a.nid=c.nid2 where a.lgnid = '"+mlgn+"' and (a.jumlah-ifnull(b.bayar,0)-ifnull(c.retur,0)) >0 "
		//alert (mcomm)
		mordd="Nota"
		mffff=" nid "
	...
	$Pcs2("#btnsimpan").click(function() {
		...
		if (mkarid==''){
		malert="TIDAK BISA DISIMPAN ! <br> Sales Kosong"
		$Pcs2("#spanket").html(malert)		
		$Pcs2("#mkarid").focus();
		return;}
		...
		success : function(data){
			...
			mq1="insert into transreturjual1(nid,tgl,jualid,ket,lgnid,total,status,lokid,sales) "
			mq2=" values('"+mnid+"',"+mtgl+",'"+mjualid+"','"+mket+"','"+mlgnid+"','"+mtotal+"',2,'"+mlokid+"','"+mkarid+"')"
			execajaxa(mq1+mq2)
			...
			msqql2="values('10310','"+mstoid+"','"+mlokid+"',"+mtgl+",'"+mnid+"',"+toval(mgggg.hrgbeli)*mtotqty+","+mtotqty+",'TRANSRETURjual','Selisih Retur ke "+mlgnnama+"')"
...
<form id="fform" name="fform">
	<table border="0" style="color: white; font-family: Arial; font-weight: bold" width="85%">
		<tr>
			<td>Tgl. </td>
			<td>: <input id="mtgl" size="10" type="text"> Kode :
			<input id="mnid" size="10" type="text"><input id="lookmnid" type="button" value="F5"></td>
		</tr>
		<tr>
			<td>pelanggan </td>
			<td colspan="5">: <input id="mlgnid" size="5" type="text"><input id="lookmlgnid" type="button" value="F5"><input id="mlgnnama" disabled size="25" type="text">
			Sales : <input type=text  size=5 id='mkarid'><input type=button id=lookmkarid value='F5'><input type=text id=mkarnama size=26 disabled>
			</td>
		</tr>
		<tr>
			<td>Atas Nota</td>
			<td>: <input id="mjualid" size="10" type="text"><input id="lookjual" type="button" value="F5"> 
			Tgl : <input id="mjualtgl" disabled size="10" type="text"> 
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Lokasi : <input type=text  size=5 id='mlokid' value='01'><input type=button id=looklok value='F5' ><input type=text id=mloknama size=26 disabled>
			</td>
		</tr>
		<tr>
			<td>Catatan </td>
			<td colspan="5">: <input id="mket" size="87" type="text"></td>
		</tr>
	</table>
	...
	<b>
	<input type='button' value='F4-Print' id='btnprint'>
	</b>
