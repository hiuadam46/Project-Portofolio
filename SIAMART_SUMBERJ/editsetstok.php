...
<table>
	<tr></tr>
	<tr><td>Lokasi</td><td align=center>ID</td><td align=center>ZONA</td><td align=center>RAK</td><td align=center>SHELF</td><td align=center>CELL</td></tr>
<?php
$query_toko = "select loknama, lokid from setlok";
$lokasi_toko=executerow($query_toko);
$iter = 0;
while ($lok1=mysql_fetch_assoc($lokasi_toko)){
	$iter = $iter + 1;
	// echo $lok1[loknama];
	$kgudang = 'kgudang'.$iter;
	$zona = 'zona'.$iter;
	$rak = 'rak'.$iter;
	$shelf = 'shelf'.$iter;
	$cell = 'cell'.$iter;
	if($mstok[$kgudang] != NULL){
		$tempat_gudang = $mstok[$kgudang];
	}
	else{
		$tempat_gudang = $lok1[lokid];
	}
	echo "<tr><td>$lok1[loknama]</td>
	<td>: <input type=text size=5 id='gudang$iter' value='$tempat_gudang' name='gudang$iter'></input></td>
	<td><input type=text size=10 id='zona$iter'  value='$mstok[$zona]' name='zona$iter'></input></td>
	<td><input type=text size=10 id='rak$iter'  value='$mstok[$rak]' name='rak$iter'></input></td>
	<td><input type=text size=10 id='shelf$iter'  value='$mstok[$shelf]' name='shelf$iter'></input></td>
	<td><input type=text size=10 id='cell$iter'  value='$mstok[$cell]' name='cell$iter'></input></td>
	</tr>";
}
$jumlah_toko = count($lokasi_toko);
// for($x=1; $x<=$jumlah_toko; $x++){
// 	echo "<tr><td></td></tr>"
// }
?>
</table>
...
<tr><td align=left>Hrg. Jual A&nbsp</td> <td colspan=3> : <input type=text size=10 name='mhrgjual' id='mhrgjual' $msty2  value='<?php echo number_format($mstok[hrgjual],0,'.',',') ?>' style='text-align:right'></td></tr>
<tr><td align=left>Hrg. Jual B&nbsp</td> <td colspan=3> : <input type=text size=10 name='mhrgjual2' id='mhrgjual2' $msty2  value='<?php echo number_format($mstok[hrgjual2],0,'.',',') ?>'  style='text-align:right'></td></tr>
<tr><td align=left>Hrg. Jual C&nbsp</td> <td colspan=3> : <input type=text size=10 name='mhrgjual3' id='mhrgjual3' $msty2  value='<?php echo number_format($mstok[hrgjual3],0,'.',',') ?>'  style='text-align:right'></td></tr>
