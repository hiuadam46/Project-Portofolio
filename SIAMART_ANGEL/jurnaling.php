...
if ($medc<>0)
{
			$sqlkartu=execute("select * from setrek where reknama='$mkartu'");
			$idkartu=$sqlkartu[rekid];

			// $sqlkartu = "select rekid from setrek where reknama ='$mkartu'";
			// $idkartu = executerow($sqlkartu);
			// $idnya = $idkartu->rekid;
			$msqql1="insert into bkbesar(rekid ,bpid,bpid2,tgl,nid,debet,trans,ket) "	;
			$msqql2="values('$idkartu','$mkartu','','$mtgl','$mnid',$medc,'TRANSKASIR','EDC')";
			execute($msqql1.$msqql2);
}
...
if ($mtunai>0)
{
			...
			$msqql1="insert into bkbesar(rekid ,bpid,bpid2,tgl,nid,debet,trans,ket) "	;
			$msqql2="values('10010','$mlgnid','','$mtgl','$mnid','$mtunai','TRANSJUAL','Penjualan')";
			execute($msqql1.$msqql2);

}
			
$moo2=executerow("select * from transjual2 where nid='$mnid'");
$mtot=0;
...
$mjm=savstok('L002',$mstoid,$mnid,$mtgl,0,0,0,$mtotqty,'TRANSJUAL',"Penjualan");
