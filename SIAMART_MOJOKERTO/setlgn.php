...
<html><head>
    ...
    <script type="text/javascript">
	  ...
	  execajaxas("alter table setlgn add npwp varchar(20) default ''")
	...
	return;	
		...
		if (! (this.value=='A' || this.value=='B' || this.value=='C' ))
		...
		else
			{
				...
				if (this.value=='A')
				{
					$Pcs2("#mKeterangan").val("PROVISION")
				}
				else
				{
					if (this.value=='B')
					{
						$Pcs2("#mKeterangan").val("WHOLE SELLER")
					}
					else
					{
						if (this.value=='C')
						{
							$Pcs2("#mKeterangan").val("SUPERMARKET")
						}
					}
				}
				//alert ($Pcs2("#mKeterangan").val())
			}
		})
...
?>
	...
	FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_mojokerto_data' and TABLE_NAME='setlgn' 
	and COLUMN_COMMENT<>''
