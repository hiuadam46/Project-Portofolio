...
<form id="dform" method="POST" onsubmit="if (!this.submitted) return false; else return true;">
	<label style="color:white">Filter <input id="mcar" type="text"> Grup&nbsp;:&nbsp;<input id="mgrpid" onblur="ambildata()" size="20" type="text"><input id="lookgrupp" onclick="lookgrup('mgrpid')" type="button" value="F5">
...
$mwd8='4 readonly';
echo("
			<th rowspan=1 colspan=5> Harga </th>
");
$mwd910111213='10 readonly';
echo("
...
$mwd9='8 readonly';
echo("
			<th rowspan=1> Spesial </th>
");
$mwd10='8 readonly';
echo("
			<th rowspan=1> Grosir </th>
");
$mwd11='8 readonly';
echo("
			<th rowspan=1> Ecer </th>
");
$mwd12='8 readonly';
echo("
			<th rowspan=1> Hrg Khusus</th>
");
$mwd13='8 readonly';
echo("
		</tr>
...
for ($ggg=1;$ggg<101;$ggg++)
{
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd3 value='".$rows[barcode]."' $nstt onblur='ambilstok(this)'></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd4 value='".$rows[barcode2]."' $nstt onblur='ambilstok(this)'></td>
	";

	$mjrow=$mjrow+1;$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd5 value='".$rows[stonama]."' $nstt></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd6 value='".$rows[satuan1]."' $nstt></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd7 value='".$rows[isi]."' $nstt></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd8 value='".$rows[satuan2]."' $nstt></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd9 value='".number_format($rows[hrgbeli],0,'.',',')."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd10 value='".number_format($rows[hrgjual],0,'.',',')."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd11 value='".number_format($rows[hrgjual2],0,'.',',')."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd12 value='".number_format($rows[hrgjual3],0,'.',',')."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=10 value='".number_format($rows[hrgjual4],0,'.',',')."' $nstt2></td>
	";

	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd14 value='0' $nstt3></td>
	";
	...
	// 	";
	//<input type="checkbox" <?= ($row['status']=='Y'?'checked':'') value="" ><?php echo $row["lokasi"]
	//03JULI2020
	$mjrow=$mjrow+1;
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "																	
			<td><input type='checkbox' $midname size=$mwd14 class='chkhk' .($rows[print]==1?'checked':'').' onclick=ambilprint(this);arah(this)></td> ";
	//03JULI2020
...
?>
	...
	<div id="printkolom" title="Pilih Kolom Untuk Cetak">
		<table width="100%">
			...
			<tr>
				<td><input id="ck12" checked type="checkbox" value="hrgjual4">Hrg. 
				Khusus </td>
			</tr>
			<tr>
	...
	$Pcs2.ajax(
	{
		...
		success : function(data)
		{
			...
			$Pcs2.each(data, function(index, array)
			{
				...
				$Pcs2("#22_"+nn).val(tra(toval(array['hrgjual3'])));
				$Pcs2("#23_"+nn).val(tra(toval(array['hrgjual4'])));
				mprint0=toval(array['print']);
				if(mprint0==1)
				{
					$Pcs2("#25_"+nn).attr('checked', true);
				}
				else
				{
					$Pcs2("#25_"+nn).attr('checked', false);
				}
...
function ambilprint(theid)
	{
		...
		if (theid.checked)		
		{ 
			execajaxa("update setstok set print=1 where stoid='"+mstoid+"'") 
		}
		else
		{
			 execajaxa("update setstok set print=0 where stoid='"+mstoid+"'") 
		}
	}
