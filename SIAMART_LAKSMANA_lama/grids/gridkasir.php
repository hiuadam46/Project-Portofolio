...
/* $query='
select a.stoid, FORMAT(a.qty, 2) qty, FORMAT(a.hrgsat, 0) hrgsat, cdiskon diskon, FORMAT(a.jmlhrg, 0) jmlhrg, a.satuan, a.isi, b.stonama, b.satuan1, b.satuan2, (a.jmlhrg/c.poin_rp) poin
from transkasir2 a
...
'; */
$query='
select a.stoid, FORMAT(a.qty, 2) qty, FORMAT(a.hrgsat, 0) hrgsat, cdiskon diskon, FORMAT(a.jmlhrg, 0) jmlhrg, a.satuan, a.isi, b.stonama, b.satuan1, b.satuan2, b.jmlhrg jmlhrg_b, (a.jmlhrg/b.jmlhrg) poin
from transkasir2 a
left join setstok b on a.stoid=b.stoid
left join setgrp c on b.grpid=c.grpid
where a.nid="'.$mnid.'" order by num
...
while ($rows=mysql_fetch_assoc($rrw))
{
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd2 value='' $nstt onblur='ambilstok(this)'></td>
	";
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td ><input type='text' $midname size=$mwd3 value='".$rows[stonama]."' $nstt></td>
	";
...
for ($mgg=$mnom;$mgg<100;$mgg++)
{
	...
	$midname="id='$mjrow".'_'."$mnom' name='$mjrow".'_'."$mnom'";
	echo "
			<td><input type='text' $midname size=$mwd2 onblur=ambilstok(this) value='' $nstt></td>
	";
