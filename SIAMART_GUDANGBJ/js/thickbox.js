...
function TB_init(){
	...
	if (t != null) {
		var h = t.indexOf('http://');
		if (h > -1) {
			var e = t.indexOf(' ', h);
			if (e == -1) 
				e = t.length;
			t = t.substring(0, h) + '<a href="' + t.substring(h, e) + '">' + t.substring(h, e) + '</a>' + t.substring(e);
		}
	}
...
function TB_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
	...
	try {
		...
		} else if (document.getElementById("TB_window") == null) {
		...
		$(document).bind('keyup', function(e){ var key = e.keyCode; if(e.keyCode == 27) TB_remove(); });
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
			...
			imgPreloader.onload = function(){
				...
				$("#TB_window").append("<a href='' id='TB_ImageOff' title='Close'><img id='TB_Image' src='"+url+"' width='"+imageWidth+"' height='"+imageHeight+"' /></a>" + "<div id='TB_caption'>"+caption+"<div id='TB_secondLine'>" + TB_imageCount + TB_PrevHTML + TB_NextHTML + "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='Close'>close</a></div>"); 		
				...
				if (!(TB_PrevHTML == "")) {
					...
					$("#TB_prev").bind('click', goPrev);
					
					$(document).bind('keyup', function(e){ var key = e.keyCode; if(key == 37){goPrev()} });
				}
				...
				if (!(TB_NextHTML == "")) {		
					...
					$("#TB_next").bind('click', goNext);
				
					$(document).bind('keyup', function(e){ var key = e.keyCode; if(key == 39){goNext()} });
				}
				
				TB_position();
				$("#TB_load").remove();
				$("#TB_ImageOff").bind('click', TB_remove);
