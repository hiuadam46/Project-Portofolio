...
<title>Pembelian Per Nota</title>
...
$Pcs2(document).ready(function()
{
	...
	buatlaporan();
	$Pcs2("#headertrans").html("<b>Pembelian Per Nota")
...
function buatlaporan(ngeprint, excel)
{
	...
	if(!excel)
	{
		if (!ngeprint)
		{
			$Pcs2("#framelap").attr("src","isilaporan.php?mLapo=mlapbelinota&mtg1="+mtgl1+"&mtg2="+mtgl2+"&mppn="+mppn+"&msupid="+asd);
		}
		else
		{
			window.open("isilaporan.php?mLapo=mlapbelinota&mtg1="+mtgl1+"&mtg2="+mtgl2+"&mppn="+mppn+"&isprint=YY"+"&msupid="+asd);
		}
	}
	else
	{
		window.open("isilaporan.php?mLapo=mlapbelinota&excel=true&mtg1="+mtgl1+"&mtg2="+mtgl2+"&mppn="+mppn+"&isprint=YY"+"&msupid="+asd);
	}
}
</script>
<script type="text/javascript">
...
{
	mppn=$Pcs2("#mppn").val()
	mcomm="Select rpad(supid,12,' ') Kode,left(supnama,50) Nama from setsup where isppn like '"+mppn+ "%'"
	mordd="Kode"
...
<form id="fform" name="fform">
	<div>
		<table style="position: relative; text-align: left; color: white; font-family: Arial" width="100%">
			<tr>
				<td>Tgl.&emsp;&emsp;&emsp;&emsp;&emsp;&nbsp; 
				<input id="mtgl1" readonly size="10" type="text"> 
			...
			<tr>
				<td colspan="2">
					...
					<select id="mppn" name="mppn" style="color: black; font-family: Arial">
						...
						<option value="PPN">SUPPLIER PPN</option>
						<option value="Non PPN">SUPPLIER NON PPN</option>
					</select>
					&emsp;
					Suplier :
					<input id="msupid" size="8" type="text">
					<input id="mlsup" type="button" value="F5">
					<input id="msupnama" readonly type="text">
				</td> 
			</tr>
		</table>
	</div>
