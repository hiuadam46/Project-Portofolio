...
$Pcs2(document).ready(function()
{
		...
		//alert(mmid)
			...
			if (mmv==116 && (mmid.substr(0,2)=='12' || mmid.substr(0,2)=='32') )
			{
				if (terusEntry())
				{
					...
					{
						query1="select left(a.stoid, 10) K, left(a.barcode, 12) Barc, left(a.stonama, 48) Nama, left(format(a.hrgjual"+mha+"/if("+msatuan+"=1, 1, isi), 0), 12) rightHarga from setstok a "
						query2=" where true "
					}
					query1="select left(a.stoid, 10) K, left(a.barcode, 12) Barc, left(a.stonama, 48) Nama, left(format(a.hrgjual"+mha+"/if("+msatuan+"=1, 1, isi), 0), 12) rightHarga, 'L001' QSALDO, stoid from setstok a "
					query2=" where true "
					mcomm=query1+query2
	...
	$Pcs2("#btnsimpan").click(function()
	{
		...
		if (mbayar<mjumlah)
		{
			datas=taptabel("setlgn","lgnid,DATE_ADD(str_to_date('"+$Pcs2("#mtgl").val()+"','%d-%c-%Y'), INTERVAL tempo DAY) jt","lgnid='"+mlgnid+"'")
			if (datas.jt !=undefined)
			{
				mjt=datas.jt
				//mjt="str_to_date('"+datas.jt+"','%d-%c-%Y')"
			}
			else 
			{
				mjt=''
		...
		{
			mkartu=GetSelectedValue()
		}
		mq1="insert into transkasir1(nid, tgl, jam, kasir, total, diskon, jumlah, edc, bayar, kembali, status, kartu, referensi1, referensi2, lgnid,tgljt) "
		mq2=" values('"+mnid+"', "+mtgl+", '"+mjam+"', '"+mkasir+"', '"+mtotal+"', '"+mdiskon+"', '"+mjumlah+"', '"+medc+"', '"+mbayar+"', '"+mkembali+"', 2, '"+mkartu+"', '"+mref1+"', '"+mref2+"', '"+mlgnid+"','"+ mjt +"')"
		execajaxas(mq1+mq2)
...
{	terus=true
	for (gg=1;gg<100;gg++)
	{
		matCode=$Pcs2("#stoid_"+gg).val()
		if(matCode!='')
		{
			if (gg==99)
			{
				//alert("jumlah item max 3= " + gg)
				terus=false
				break;
				//return;
			}
...
function arahharga(theid,golikut)
{
		...
		{
			//coba tambah 20MAR20
			if(mgolongan=='D' || mgolongan == '' && golikut==undefined){
				mharga=3
			}
			//coba tambah 20MAR20
...
function baru(mgdn)
{
	...
	$Pcs2("#lblwalk").html("CV. WPS Siap Melayani Anda")
		...
		success : function(data)
		{
			...
			if (mstok.stonama!=undefined )
			{
				...
				if (misi==mstok.barcode2)
				{
					...
					//alert("masuk kondisi =barcode2, var harga = "+ mharga)
				}
				...
				if (misi==mstok.barcode)
				{
					...
					mfield="stoid, barcode, barcode2, stonama, FORMAT(hrgbeli/if("+msatuan+"=1, 1, isi), 0) hrgbeli, satuan"+msatuan+" satuan, FORMAT(hrgjual"+mharga+"/if("+msatuan+"=1, 1, isi), 0) hrgjual, 0 diskon, if("+msatuan+"=1, isi, 1) isi, b.poin_rp"
					mstok=taptabel(mtabel, mfield, mkondisi)
					//alert("masuk kondisi =barcode, var harga = "+ mharga)
				}
...
function GetSelectedValue(){
	var e = document.getElementById('mkartu');
...
<form id="fform" name="fform">
	<table border="0" style="border-collapse: ; color: white; font-family: Arial; font-weight: bold" width="100%">
		...
		<tr>
			...
			<label id="lblwalk" style="font-size: 18pt">ABILIYAN SWALAYAN dan GROSIR 
		...
		<tr>
			...
			<table id="mtab1" border="0" style="font-size: 12pt; background-color: #A52A2A" width="100%">
				...
				<tr>
					<td align="right" colspan="2">Kartu&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
					<?php
						$sql = "select rekid,reknama from setrek where nrcid='101'";
						$query = mysql_query($sql);
						echo '<select id="mkartu">';
						echo '<option value="">-pilih kartu-</option>';
