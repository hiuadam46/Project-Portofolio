...
<td hidden><input type='text_$mnom' id='5_$mnom' name='5_$mnom' class='rcell' size=12 value='".$rows[bpid]."' disabled $msty  onblur=ambilbp(this)></td>
<td hidden><input type='text' id='6_$mnom' name='6_$mnom' class='rcell'  size=20 value='".$rows[bpnama]."' disabled $msty></td>
</tr>
...
$query="
select a.rekid,b.reknama,FORMAT(a.debet+a.kredit,0) jumlah,a.bpid,concat(ifnull(c.supnama,''),ifnull(d.karnama,''),ifnull(e.lgnnama,'')) bpnama 
from bkbesar a  
...
where a.nid='$mnnid' and nid2='O' order by rekid
";
	...
	while ($rows=mysql_fetch_assoc($rrw))
	{
		...
		<td hidden><input type='text_$mnom' id='5_$mnom' name='5_$mnom' class='rcell' size=12 value='".$rows[bpid]."' $msty  onblur=ambilbp(this)></td>
		<td hidden><input type='text' id='6_$mnom' name='6_$mnom' class='rcell'  size=20 value='".$rows[bpnama]."' disabled $msty></td>
		</tr>";
		$mnom++;
	}
	...
	for ($mii=$mnom;$mii<100;$mii++)
	{
		...
		<td hidden><input type='text_$mnom' id='5_$mnom' name='5_$mnom' class='rcell' size=12 value='' $msty' onblur=ambilbp(this)></td>
		<td hidden><input type='text' id='6_$mnom' name='6_$mnom' class='rcell'  size=20 value='' disabled $msty'></td>
		</tr>";
		$mnom++;
	}
	$mjum=$mnom-1;
