...
function looktrans(mnid)
{
	...
	if (mnd=='TP')
	{
		mswi="transproduksi.php?mnid="+mnid
	}
	if (mnd=='TB')
	{
	...
	if (mnd=='TJ')
	{
		mswi="transjual.php?mnid="+mnid
	}
	if (mnd=='TK')
	{
		mswi="transkasir.php?mnid="+mnid
	}
	if (mnd=='RJ')
	{
		mswi="transreturjual.php?mnid="+mnid
	}
	if (mnd=='LP')
	{
		mswi="translunaspiutang.php?mnid="+mnid
	}
	if (mnd=='MG')
	{
		mswi="transmutasigudang.php?mnid="+mnid
	}
	window.open(mswi)
}
...
{
	$sqlstr="
	select a.*, b.lgnnama, date_format(a.tgl, '%d-%m-%Y') tglc
	from transkasir1 a
	left join setlgn b on a.lgnid=b.lgnid
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and total<>0 and a.lgnid like '!!$mlgnid!!' and (a.lgnid='' or b.telp2 like '$mpn!!')
	order by a.tgl, a.jam, a.nid
	";
	...
	while ($mrr=mysql_fetch_object($maa))
	{
		...
		// $mss=executerow('
		// select a.*, date_format(a.tgl, "%d-%m-%Y") tglc, b.stonama, b.satuan1, b.satuan2, b.grpid, e.grpnama, d.lgnnama
		// from transkasir2 a
		...
		// ');
		$mss=executerow('
		select a.*, b.stonama, b.satuan1, b.satuan2, b.grpid, e.grpnama
		from transkasir2 a
		left join setstok b on a.stoid=b.stoid
		left join transkasir1 c on a.nid=c.nid
		left join setgrp e on e.grpid=b.grpid
		...
		$mnomor=1;
		$mtotqty=0;
		$jmlbrs=mysql_num_rows($mss);

		// diblok dulu krn isi group yg kosong jadi kacau
		...
		//coba dipindahkesini
		if ($jmlbrs==0) {$jmlbrs=1;}
		echo '
		<tr>
			<td rowspan= '.$jmlbrs.' >'.$mrr->tglc.'.</td>
			<td rowspan= '.$jmlbrs.'>'.$mrr->nid.'.</td>
			<td rowspan= '.$jmlbrs.'>'.$mrr->lgnnama.'</td> ';
		// //coba dipindahkesini
		...
		while ($msx=mysql_fetch_object($mss))
		{
			...
			<td align=right>'.number_format($msx->jmlhrg-$PPN,0,',','.').'</td>
	...
	echo '
		<tr valign=top>
			...
			<td align=right><b>'.number_format($total_GT-$PPN_GT,0,',','.').'</td>
			<td align=right><b>'.number_format($PPN_GT,0,',','.').'</td>
			<td align=right><b>'.number_format($total_GT,0,',','.').'</td>
		</tr>
	</table>
	<br><br>
	';
...
from setstok a
left join (select bpid,sum(debet-kredit) awal,sum(qtyin-qtyout) awalq from bkbesar where tgl<str_to_date('$mtg1','%d-%c-%Y') and rekid like '103%' and bpid2 like '$mlokid%' group by bpid) b on a.stoid=b.bpid
left join (select bpid,sum(kredit) kredit,sum(debet) debet,sum(qtyin) qtyin,sum(qtyout) qtyout from bkbesar where
rekid like '103%' and tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') and bpid2 like '$mlokid%'
...
<td bgcolor=#FFFFFF align=right><font size=2>$mdebet</font></td>
<td bgcolor=#FFFFFF align=right><font size=2>$mkreditq</font></td>
<td bgcolor=#FFFFFF align=right><font size=2>$mkredit</font></td>
...
///////////////BEGINNING OF LAPORAN KARTU STOK/////////////////////
if ($mLapo=='mkrtstok')
{
...
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_wps_data' and TABLE_NAME='$mtabelnya'
";
...
{
$msql="SELECT COLUMN_NAME kolom,DATA_TYPE tipe,NUMERIC_PRECISION panjang1,CHARACTER_MAXIMUM_LENGTH panjang2,COLUMN_COMMENT koment
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='siamart_wps_data' and TABLE_NAME='$mtabelnya' and COLUMN_NAME in ($mkol)
";
}
...
//////////////BEGINNING OF LAPORAN PENJUALAN DETAIL//////////////////////
if ($mLapo=='mlapjual2')
{
// aslinya
...
// $sqlstr="select a.*,b.lgnnama
// from transjual1 a left join setlgn b on a.lgnid=b.lgnid
// left join transjual2 c on a.nid=c.nid 
// left join setstok d on c.stoid=d.stoid
// where a.tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y')
// and a.lgnid like '!!$mlgnid!!' and d.grpid like '!!$mgrpid!!' order by a.nid,a.tgl
// ";

$lgn=execute("select lgnnama from setlgn where lgnid='$mlgnid' ");
$grp=execute("select grpnama from setgrp where grpid='$mgrpid' ");
echo("
CV. WPS
<br>
Laporan Penjualan
<br>
<br>Tgl. $mtg1 s.d. $mtg2 $mtgt
<br>Pelanggan : $lgn[lgnnama]
<br>Group : $grp[grpnama]
<br><br>
");
...
$mss=executerow("select a.*,b.stonama,b.satuan1 from transjual2 a left join setstok b on a.stoid=b.stoid left join setstok c on b.stoid=c.stoid where a.nid='$mrr[nid]' and c.grpid like '!!$mgrpid!!' order by nomor");
$mnomor=1;
$mtotqty=0;
...
//////////////END OF LAPORAN PENJUALAN DETAIL//////////////////////
...
$sqlstr="select a.*,a.ongkos ongkosdiskon,b.supnama,date_format(tgl,'%d-%m-%Y') tglc,date_format(tgljt,'%d-%m-%Y') tgljtc,date_format(tgljt,'%d') tgljth,c.retur,d.lunas
from transbeli1 a left join setsup b on a.supid=b.supid
...
where tgl between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y')
and b.isppn like '$mppn%' and a.supid like '%$msupid%' 
...
<td>$mrr->supid</td>
<td>$mrr->supnama</td> 
<td><a href onclick=looktrans('$mrr->nid')> $mrr->nid </a></td> 
...
<td align=right>".number_format($mrr->total-$mrr->tunai-$mrr->retur-$mrr->lunas,0,'.',',')."</td> 
</td>
</tr>
";
...
$totsisa=$totsisa+($mrr->total-$mrr->tunai-$mrr->retur-$mrr->lunas);
$nn++;
}
...
if ($mLapo=='mlaplunaspiutang')
{
	...
	$sqlstr= "
	select a.*, b.lgnnama, c.nid jualid, c.tgl tgljual, ifnull(c.debet,0) debetc
	FROM 
	(
		select x.*, y.cara_bayar
		FROM bkbesar x,
		(
			SELECT nid,
			CASE
				WHEN SUBSTRING( rekid, 1, 3 ) = '101' THEN 'Transfer'
				WHEN rekid = '10010' THEN 'Tunai'
				WHEN rekid = '10220' THEN 'Giro'
			END AS cara_bayar
			FROM bkbesar
			WHERE tgl BETWEEN STR_TO_DATE('$mtg1', '%d-%c-%Y' ) AND STR_TO_DATE('$mtg2', '%d-%c-%Y' )
		) y 
		WHERE x.nid = y.nid AND cara_bayar IS NOT null
	) a 
	LEFT JOIN setlgn b ON a.bpid = b.lgnid 
	LEFT JOIN
	(
		SELECT bpid, nid, tgl, ifnull(debet,0) debet
		FROM bkbesar
		WHERE rekid = '10210' AND debet <>0
	) c ON a.nid2 = c.nid AND a.bpid = c.bpid
	WHERE a.tgl BETWEEN STR_TO_DATE('$mtg1', '%d-%c-%Y' ) AND STR_TO_DATE('$mtg2', '%d-%c-%Y' ) AND a.bpid LIKE '%$mrute%' AND a.rekid = '10210' AND a.trans = 'TRANSLUNASPIUTANG' and a.cara_bayar LIKE '%$mjnsbayar%' AND kredit <>0
...
<table width=97% cellspacing=0 cellpadding=0 bgcolor=white border=1 style='border-collapse:collapse;font-size:85%'>
	<tr>
		...
		<td bgcolor=#CCCCCC align=center colspan=5><b>Jumlah Pelunasan</b></td>
	...
	<tr>
		...
		<td bgcolor=#CCCCCC align=center><b>Bank</b></td>
	...
	while ($row = mysql_fetch_assoc ($result))
	{
		...
		$rowp=execute($sql2);
		$msaldo=$row[debetc]-$rowr[kredit]-$rowp[kredit];
		...
		$sql6="
		select sum(ifnull(a.debet,0)) transfer, b.reknama
		from bkbesar a
		left join setrek b on a.rekid=b.rekid
		where a.rekid like '101%' and a.nid='$row[nid]'
		";//TRANSFER
	...
	<tr>
		...
		<td bgcolor=white align=right>".number_format($row[debetc],0,'.',',')."&nbsp;</td>
		...
		<td bgcolor=white align=left rowspan=".$jumm.">&nbsp;".$row5[reknama]."&nbsp;</td>
	...
	<tr>
		...
		<td bgcolor=white align=right><b>&nbsp;</td>
...
order by a.nid,a.tgl
";
// $sqlstr="select c.subtotal,a.*,b.supnama,date_format(tgl,'%d-%m-%Y') tglc,date_format(tgljt,'%d-%m-%Y') tgljtc,
// date_format(tgljt,'%d') tgljth 
// from transbeli1 a left join setsup b on a.supid=b.supid 
// left join (select max(nid) nid,sum(x.jmlhrg) subtotal from transbeli2 x,setstok y 
// where y.stoid like '%$mstoid%' and x.stoid=y.stoid group by x.nid) c
// on a.nid=c.nid where 
// tgljt between str_to_date('$mtg1','%d-%c-%Y') and str_to_date('$mtg2','%d-%c-%Y') 
// and a.supid like '!!$msupid!!' order by a.nid,a.tgl ";
...
if ($mLapo=='mlapjualperiodik')
{
	...
	$saa=execute("select * from setkaryawan where karid='$mkarid'");
	...
	while ($mrr=mysql_fetch_object($maa))
	{
		...
		$totPIUTANG=$totPIUTANG+$mrr->total;
		$tottunai=$tottunai+$mrr->tunai;
		$totot=$totot+$mrr->jumlah;
		$totretur=$totretur+$mrr->retur;
		$totlunas=$totlunas+$mrr->lunas;
		$totsisa=$totsisa+($mrr->jumlah-$mrr->retur-$mrr->lunas);
...
/////////////////// BEGINNING OF LAPORAN PENJUALAN PER NOTA////////////////////
if ($mLapo=='mlapjualnota')
{
	...
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and a.lgnid like '!!$mlgnid!!' and a.kasir in (select karid from setkaryawan where karnama like '!!$mkarid!!') and $ttk and (a.lgnid='' or b.telp2 like '$mpn!!')
	order by a.nid, a.tgl
	";
...
/////////////////// END OF LAPORAN PENJUALAN PER NOTA////////////////////
...
if ($mLapo=='mlapdistribusi')
{
	...
	// 	WHERE setstok.stoid like '!!$mstoid!!'
	// ) c ON c.nid=a.nid
	// LEFT JOIN setkaryawan f ON a.sales=f.karid
	// WHERE c.tgl between str_to_date('$mtg1', '%d-%c-%Y') and 
	// str_to_date('$mtg2', '%d-%c-%Y') and a.lgnid like '!!$mlgnid!!' 
	// and a.sales like '!!$mkarid!!' and c.supid like '%$msupid%'
	// order by a.nid, a.tgl
	// ";

$sqlstr=" SELECT b.lgnid, b.lgnnama, c.stoid, c.stonama, c.hrgsat, c.qty, c.diskonrp, 
jmlhrg, c.isi, c.nid, date_format(c.tgl,'%d-%m-%Y') tgl, c.satuan1, c.satuan2, c.supid 
FROM transkasir1 a LEFT JOIN setlgn b ON a.lgnid=b.lgnid 
INNER JOIN ( SELECT transkasir2.*, setstok.satuan1, setstok.satuan2, setstok.stonama, 
setstok.supid FROM transkasir2 inner join setstok on transkasir2.stoid=setstok.stoid 
WHERE setstok.stoid like '%$mstoid%' ) c ON c.nid=a.nid 
LEFT JOIN setkaryawan f ON a.kasir=f.karid 
WHERE c.tgl between str_to_date('$mtg1', '%d-%c-%Y') 
and str_to_date('$mtg2', '%d-%c-%Y') and a.lgnid like '%$mlgnid%' and a.kasir like '%$mkarid%' 
and c.supid like '%$msupid%' order by a.nid, a.tgl ";
...
$sqlsup = "
	...
	where supid = '$msupid'
	";
	$rowsup_3207 = executerow($sqlsup);
	$rowsup = mysql_fetch_assoc ($rowsup_3207);
	...
	where lgnid = '$mlgnid'
	";
	$rowlgn_3215 = executerow($sqllgn);
	$rowlgn = mysql_fetch_assoc ($rowlgn_3215);
	...
	where stoid = '$mstoid'
	";
	$rowsto_3223 = executerow($sqlsto);
	$rowsto = mysql_fetch_assoc ($rowsto_3223);
	...
	where karid = '$mkarid'
	";
	$rowkar_3231 = executerow($sqlkar);
	$rowkar = mysql_fetch_assoc ($rowkar_3231);
...
///////////////END OFLAP DISTRIBUSI////////////////////////

if ($mLapo=='mlapjualkiriman')
{
	$sqlstr="
	select a.bpid stoid, sum(a.qtyout-a.qtyin) qty, sum(a.kredit-a.debet) hpp, b.stonama, b.isi, b.satuan1, b.satuan2
	from bkbesar a
	left join setstok b on a.bpid=b.stoid
	left join transjual1 c on a.nid=c.nid
	where a.tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and c.lgnid like '!!$mlgnid!!' and a.trans='TRANSKASIR' and a.rekid like '103%'
	group by a.bpid
	order by a.bpid
	";

	$sqlstr="
	select a.stoid, sum( ( a.qty*a.isi)+a.unit ) qty, sum(a.jmlhrg) jmlhrg, b.stonama, b.isi,b.satuan1, b.satuan2
	from transjual2 a
	left join setstok b on a.stoid=b.stoid
	left join transjual1 c on a.nid=c.nid
	where a.tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and c.lgnid like '!!$mlgnid!!'
	group by a.stoid
	order by b.stonama
	";
	...
	{
		$wrr_3394=executerow("
		select sum(kredit) hpp
		from bkbesar
		where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y') and bpid='$msx->stoid' and rekid='10310' and kredit<>0 and trans='transjual' and nid in
		(
			select nid
			from transjual1 c
			where c.lgnid like '!!$mlgnid!!'
		)
		");
		$wrr=mysql_fetch_object($wrr_3394);
	...
	$wrr2_3415=executerow("
	select sum(jumlah) jumlah
	from transjual1
	where tgl between str_to_date('$mtg1', '%d-%c-%Y') and str_to_date('$mtg2', '%d-%c-%Y')
	");
	$wrr2=mysql_fetch_object($wrr2_3415);
	...
	echo "
		...
		<tr valign=>
			<td colspan=4 align=right><b>PEMBULATAN&nbsp&nbsp</td>
			<td align=right><b>".number_format($wrr2->jumlah-$mtotal,0,'.',',')."</td>
			<td align=right><b>".number_format(0,0,'.',',')."</td>
			<td align=right><b>".number_format(0,0,'.',',')."</td>
		</tr>

		<tr valign=>
			<td colspan=4 align=right><b>TOTAL SETELAH PEMBULATAN&nbsp&nbsp</td>
			<td align=right><b>".number_format($wrr2->jumlah,0,'.',',')."</td>
			<td align=right><b>".number_format($mtotal2,0,'.',',')."</td>
			<td align=right><b>".number_format($wrr2->jumlah-$mtotal2,0,'.',',')."</td>
		</tr>
	</table>
	";
}
