...
$Pcs2(document).ready(function()
{
	...
	{
		mnid=$Pcs2("#mnid").val()
		mgg=taptabel("transjual1 a left join transpoin1 b on b.nid=a.nid", "a.*, b.poin_d", "a.nid='"+mnid+"'")
	...
	$Pcs2("#btnsimpan").click(function()
	{
		...
		execajaxa(msqql1+msqql2)
		*/
		xxw=window.open('printjual.php?mnid='+mnid+'&&mspoin='+datap.spoin, 'aa', ('alwaysraised=yes, scrollbars=no, resizable=no'));
		xxw.focus();
	...
	{
		//alert(mnid);
		if(mnid=$Pcs2("#mnid").val()=='')
		{
			return;
		}
		xsj=window.open('printsuratjalan.php?mnid='+mnid, 'aa', ('alwaysraised=yes, scrollbars=no, resizable=no'));
		xsj.focus();
	})
...
function arah(theid)
{
	...
	if (event.keyCode==13 || event.keyCode==39)
	{
		if(mdua==16)
		{
			...
			document.getElementById('12_'+mbawah).focus();
			return;
		}
...
function refreshgrid2(mgg)
{
	...
	//$mtr2=execute("select count(*) as jmlrec from transjual2 where nid = '$mnid'");
	...
	$Pcs2.ajax(
	{
		...
		success : function(data)
		{
			...
			for (abd=1;abd<100;abd++)
			{
				mpo=$Pcs2("#po_"+abd).val();
				mtp=mtp+toval(mpo);
			...
			{
				$Pcs2("#mtpoin").val(tra(mgg.poin_d));
			}
...
function ambilstok(theid)
{
	...
	rowjmhg=mmid.replace('12','24');
	rowpoin=mmid.replace('12','po');
	...
	mstok=taptabel("setstok a left join setgrp b on b.grpid=a.grpid", "a.stoid, a.stonama, a.satuan1, a.satuan2, a.satuan3, format(a.hrgjual"+mh+", 0) hrgjual, a.isi, b.poin_rp", "a.stoid='"+misi+"'");
	...
	if (mstok.stonama!=undefined)
	{
		...
		$Pcs2("#"+rowpoin).val($Pcs2("#"+rowjmhg).val()/mstok.poin_rp);
...
function rumus1()
{
	...
	for (gg=1;gg<100;gg++)
	{
		if ($Pcs2("#12_"+gg).val()!='')
		{
			...
			mstok=taptabel("setstok a left join setgrp b on b.grpid=a.grpid", "a.stoid, a.stonama, a.satuan1, a.satuan2, a.satuan3, a.isi, b.poin_rp", "a.stoid='"+$Pcs2("#12_"+gg).val()+"'");
			$Pcs2("#po_"+gg).val(toval(mnii)/mstok.poin_rp);
			mp1g=$Pcs2("#po_"+gg).val();
			mp1=mp1+toval(mp1g);
	...
	if($Pcs2("#mlgnid").val()!='')
	{
		$Pcs2("#mtpoin").val(tra(mp1.toFixed(2)));
	}
	else
	{
		$Pcs2("#mtpoin").val(0);
...
<form id="fform" name="fform">
	...
	<input id="btncetak" type="button" value="F4-Cetak Surat Jalan">
